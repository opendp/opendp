...
@pytest.mark.parametrize(
    ...
    [
        ...
        for f in [getattr(m, f_name) for f_name in dir(m) if f_name.startswith("then_")]
    ],
)
def test_thens_are_documented(module, function):
    ...
    assert f":py:func:`~{m_name}.{make_name}`" in function.__doc__, (
        f"no link to {make_name}"
    )
...
@pytest.mark.parametrize(
    "rst_path", list(docs_source.glob("**/*.rst")), ids=get_self_and_parent
)
    ...
    for i, line in enumerate(rst_lines):
        ...
        if m:
            ...
            if language not in expected:
                errors.append(
                    f'line {i + 1}: Got "{language}", expected one of: {", ".join(expected)}'
                )
    assert not errors, "\n".join(errors)
...
def test_doctest_empty_lines(path: Path):
    ...
    for i, line in enumerate(lines):
        if re.search(r">>>\s*$", line):
            errors.append(
                f'line {i + 1}: wrap with "code::" block and drop empty ">>>".'
            )
    assert not errors, "\n".join(errors)


@pytest.mark.parametrize(
    "nb_path", list(docs_source.glob("**/*.ipynb")), ids=lambda path: path.name
)
def test_notebooks_are_executed(nb_path):
    nb = loads(nb_path.read_text())
    counts_sources = [
        (cell.get("execution_count"), "".join(cell.get("source", "")))
        for cell in nb["cells"]
        if cell["cell_type"] == "code"
    ]
    triples = [
        (index, count, source)
        for (index, (count, source)) in enumerate(counts_sources, start=1)
    ]
    ...
    short_path = (
        nb_path.relative_to(Path.cwd(), walk_up=True)
...
def is_public_and_not_top(path: Path):
    ...
    return (
        ...
        and (not path.name.startswith("_") or path.name == "__init__.py")
...
public_extras = [
    path
    for path in Path(__file__).parent.parent.glob("src/opendp/extras/**/*.py")
...
@pytest.mark.parametrize("py_path", public_extras, ids=get_namespace)
