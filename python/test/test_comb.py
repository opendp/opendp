...
def test_fix_delta():
    ...
    m_gauss = dp.c.make_zCDP_to_multiDP(dp.m.make_gaussian(*input_space, 10.0))
    assert m_gauss.map(1.0).epsilon(1e-6) == pytest.approx(0.42994146883695367, abs=1e-14)

    m_gauss_fixed = dp.c.make_fix_delta(m_gauss, 1e-6)
    assert m_gauss_fixed.map(1.0) == pytest.approx((0.42994146883695367, 1e-06), abs=1e-14)
...
def test_make_composition_approx():
    ...
    composed_fixed = dp.c.make_composition(
        [
            dp.c.make_fix_delta(
                dp.c.make_zCDP_to_multiDP(dp.m.make_gaussian(*input_space, 1.0)), 1e-7
            )
        ]
        * 2
    )
    assert composed_fixed.map(1.0) == pytest.approx((11.341717670937626, 2e-07), abs=1e-14)


def test_cast_zcdp_approxdp():
    input_space = dp.atom_domain(T=float, nan=False), dp.absolute_distance(T=float)

    base_gaussian = input_space >> dp.m.then_gaussian(10.0, MO=dp.zCDP)
    assert base_gaussian.map(1.0) == 0.005

    approx_gaussian = dp.c.make_zCDP_to_multiDP(base_gaussian)
    assert approx_gaussian.map(1.0).epsilon(1e-6) == pytest.approx(0.42994146883695367, abs=1e-14)
...
def test_cast_azcdp_approxdp():
    ...
    m_asdp = dp.c.make_zCDP_to_multiDP(m_azcdp)
...
def test_make_approximate():
    ...
    m_comp = dp.c.make_composition(
        [
            m_lap,
            dp.c.make_fix_delta(
                dp.c.make_zCDP_to_multiDP(dp.m.make_gaussian(*input_space, 1.0)),
                delta=1e-6,
            ),
        ]
    )
...
def test_make_approxDP_to_multiDP():
    input_space = dp.atom_domain(T=float, nan=False), dp.absolute_distance(T=float)
    fadp_meas = dp.c.make_approximate(dp.m.make_laplace(*input_space, 10.0))

    adp_meas = dp.c.make_approxDP_to_multiDP(fadp_meas)
