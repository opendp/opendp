...
def test_fix_delta():
    ...
    assert m_gauss.map(1.).epsilon(1e-6) == 0.42994146883694934

    m_gauss_fixed = dp.c.make_fix_delta(m_gauss, 1e-6)
    assert m_gauss_fixed.map(1.) == (0.42994146883694934, 1e-06)
...
def test_make_basic_composition_approx():
    ...
    assert composed_fixed.map(1.) == (11.341717670937543, 2e-07)
...
def test_cast_zcdp_approxdp():
    ...
    assert base_gaussian.map(1.) == 0.005

    approx_gaussian = dp.c.make_zCDP_to_approxDP(base_gaussian)
    assert approx_gaussian.map(1.).epsilon(1e-6) == 0.42994146883694934
