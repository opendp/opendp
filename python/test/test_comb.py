...
def test_fix_delta():
    ...
    m_gauss = dp.c.make_zCDP_to_profileDP(dp.m.make_gaussian(*input_space, 10.0))
...
def test_make_composition_approx():
    ...
    composed_fixed = dp.c.make_composition(
        [
            dp.c.make_fix_delta(
                dp.c.make_zCDP_to_profileDP(dp.m.make_gaussian(*input_space, 1.0)), 1e-7
            )
        ]
        * 2
    )
...
def test_cast_zcdp_approxdp():
    ...
    approx_gaussian = dp.c.make_zCDP_to_profileDP(base_gaussian)
...
def test_cast_azcdp_approxdp():
    ...
    m_asdp = dp.c.make_zCDP_to_profileDP(m_azcdp)
...
def test_make_approximate():
    ...
    m_comp = dp.c.make_composition(
        [
            m_lap,
            dp.c.make_fix_delta(
                dp.c.make_zCDP_to_profileDP(dp.m.make_gaussian(*input_space, 1.0)),
                delta=1e-6,
            ),
        ]
    )
...
def test_make_approxDP_to_profileDP():
    ...
    adp_meas = dp.c.make_approxDP_to_profileDP(fadp_meas)
