...
def test_np_array2_domain():
    ...
    with pytest.raises(ValueError):
        with optional_dependency('numpy'):
            dp.x.np_array2_domain(p=2, T=float)
    # origin is wrong type
    with pytest.raises(ValueError):
        dp.x.np_array2_domain(norm=1, p=2, origin="a", T=float)
    # scalar origin must be at zero
    with pytest.raises(ValueError):
        dp.x.np_array2_domain(norm=1, p=2, origin=2, T=float)
    ...
    with pytest.raises(ValueError):
        dp.x.np_array2_domain(norm=1, p=2, origin=np.array([True, False]))

    # origin defaults to zero
    assert dp.x.np_array2_domain(norm=1, p=2, T=float).descriptor.origin == 0
    # when num columns known, origin defaults to zero vector
    domain = dp.x.np_array2_domain(norm=1, p=2, num_columns=4)
    assert np.array_equal(domain.descriptor.origin, np.zeros(4))
    assert domain.member(np.array([[1.0, 0.0, 0.0, 0.0]]))

    domain = dp.x.np_array2_domain(norm=1, p=2, origin=np.array([1, 2]), T=float)
    assert domain.descriptor.num_columns == 2
    assert domain.descriptor.origin.dtype.kind == "f"

    assert dp.x.np_array2_domain(T=bool).member(np.array([[True, False]]))
