...
def test_np_sum():
    ...
    with optional_dependency('numpy'):
        space = dp.numpy.np_array2_domain(norm=1., p=2, T=float), dp.symmetric_distance()
    trans = space >> then_np_sum()
    assert trans.map(1) == 1

    # sized data
    space = dp.numpy.np_array2_domain(norm=1., p=2, size=1000, T=float), dp.symmetric_distance()
    trans = space >> then_np_sum()
