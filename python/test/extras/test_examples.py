...
def test_no_shell_http(ext, cmd):
    paths = list(
        (Path(__file__).parent.parent.parent.parent / "docs" / "source").glob(
            f"**/*.{ext}"
        )
    )
    assert len(paths) > 0
    ...
    for path in paths:
        for n, line in enumerate(path.read_text().splitlines()):
            match = re.search(rf"\b{cmd}\W+\w\S+", line)
            if match:
...
def test_france_lfs():
    assert (
        ...
        .read_text()
        .startswith("PIDENT,YEAR,QUARTER,SEX,AGE,HWUSUAL,HWACTUAL,ILOSTAT\n")
    )
