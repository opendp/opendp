...
from opendp.extras.sklearn.cluster._tree import (
    ...
    Node,
    Quadtree,
    _axis_score_sensitivity,
    _child_splitters,
    _inverse_variance_weight,
    _leaf_penalty,
    _normalize,
    _noisy_max_scale_for_budget,
    _noise_var,
...
def test_noisy_max_scale_for_budget_matches_requested_budget():
    sensitivity = _axis_score_sensitivity(
        Quadtree(np.array([0.0, 0.0]), np.array([5.0, 5.0])),
        "kmeans",
    )
    scale = _noisy_max_scale_for_budget(
        dp.max_divergence(),
        0.25,
        sensitivity=sensitivity,
    )
    m = dp.m.make_noisy_max(
        input_domain=dp.vector_domain(dp.atom_domain(T=float, nan=False), size=2),
        input_metric=dp.linf_distance(T=float),
        output_measure=dp.max_divergence(),
        scale=scale,
    )
    assert np.isclose(sensitivity * m.map(1), 0.25)


def test_kmedians_axis_score_sensitivity_matches_l1_box_diameter():
    splitter = Quadtree(np.array([0.0, -1.0]), np.array([2.0, 3.0]))
    sensitivity = _axis_score_sensitivity(splitter, "kmedians")
    assert sensitivity == pytest.approx(6.0)
