...
def test_count_by():
    ...
    from opendp.trans import make_split_dataframe, make_select_column, make_resize, make_sized_count_by
    ...
    laplace_histogram_from_dataframe = binary_search_chain(
        lambda s: (
            preprocess >>
            make_sized_count_by(size=size, MO=L1Distance[float], TIA=str) >>
    ...
    gaussian_histogram_from_dataframe = binary_search_chain(
        lambda s: (
            preprocess >>
            make_sized_count_by(size=size, MO=L2Distance[float], TIA=str) >>
