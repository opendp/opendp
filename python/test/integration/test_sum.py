...
def test_sized_bounded_float_sum():
    ...
    preprocess = (
        ...
        dp.t.then_impute_constant(constant=0.0)
        >>
        # Clamp values
        dp.t.then_clamp(bounds=bounds)
        >>
        # Resize dataset length
        dp.t.then_resize(size=size, constant=0.0)
        >>
        # Aggregate with sum
        dp.t.then_sum()
    )
    ...
    gaussian_known_n_sum_from_dataframe = dp.binary_search_chain(
        lambda s: dp.c.make_fix_delta(dp.c.make_zCDP_to_multiDP(preprocess >> dp.m.then_gaussian(s)), 1e-5),
        d_in=1, d_out=(1., 1e-5))

    assert laplace_known_n_sum_from_dataframe.check(1, 1.0)

    data = "\n".join(["1"] * size)

    print(
        "laplace_known_n_sum_from_dataframe(data)",
        laplace_known_n_sum_from_dataframe(data),
    )
    print(
        "gaussian_known_n_sum_from_dataframe(data)",
        gaussian_known_n_sum_from_dataframe(data),
    )
...
def test_sized_bounded_int_sum():
    ...
    assert noisy_known_n_sum_from_dataframe.check(1, 1.0)

    data = "\n".join(["1"] * size)

    print(
        "noisy_known_n_sum_from_dataframe(data)", noisy_known_n_sum_from_dataframe(data)
    )
...
def test_bounded_float_sum():
    ...
    assert laplace_sum_from_dataframe.check(1, 1.0)
...
def test_bounded_int_sum():
    ...
    assert noisy_sum_from_dataframe.check(1, 1.0)
