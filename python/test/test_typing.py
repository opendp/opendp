...
def test_typing_infer_to_string():
    ...
    assert RuntimeType.infer(12.0) is "f64"  # noqa: F632
...
def test_typing_infer_to_object():
    ...
    with pytest.raises(
        TypeError, match=re.escape("elements must be homogeneously typed")
    ):
    ...
    infer_tuple = RuntimeType.infer((12.0, True, "A"))
    ...
    with pytest.raises(
        ...
        match=re.escape("Type of Option cannot be inferred from None"),
    ):
        RuntimeType.infer(None)
    with pytest.raises(
        UnknownTypeException, match=re.escape("Cannot infer atomic type when empty")
    ):
        RuntimeType.infer([])
...
def test_tuples():
    ...
    assert (
        str(RuntimeType.parse(("f32", (int, "L1Distance<i32>"))))
        == "(f32, (i32, L1Distance<i32>))"
    )
    assert (
        str(RuntimeType.parse(("f32", (int, L2Distance[float]))))
        == "(f32, (i32, L2Distance<f64>))"
    )
...
@pytest.mark.parametrize(
    "integer_type", set(_INTEGER_TYPES) - disallowed_int_default_types
)
