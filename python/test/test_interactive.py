...
def test_fully_adaptive_composition():
    ...
    o_comp = space >> dp.c.then_fully_adaptive_composition(dp.max_divergence(), max_influence)
    ...
    assert qbl_comp.privacy_loss() == 0.0

    assert str(qbl_comp) == "OdometerQueryable(Q=AnyMeasurement)"
    m_sum = space >> dp.t.then_clamp((0, 10)) >> dp.t.then_sum() >> dp.m.then_laplace(100.)

    # evaluating
    assert isinstance(qbl_comp(m_sum), int)
    assert qbl_comp.privacy_loss() == m_sum.map(max_influence)
    ...
    assert qbl_comp.privacy_loss() > m_sum.map(max_influence) + 0.2 + 0.09
    ...
    # it's slightly larger, checking greater than will do
    assert qbl_comp.privacy_loss() > m_sum.map(max_influence) * 2 + 0.2 + 0.09
...
def test_privacy_filter():
    ...
    qbl_filter(m_count)
    assert qbl_filter.privacy_loss() == 1.0
    qbl_filter(m_count)
    assert qbl_filter.privacy_loss() == 2.0
...
def test_odometer_chain_ot():
    ...
    sc_qbl2: dp.OdometerQueryable = sc_odo2(["1"] * 200)
...
def test_odometer_chain_po():
    ...
    # a transformation postprocessor reduces to just a function
    postprocessor = dp.t.make_user_transformation(
        input_domain=dp.atom_domain(T=int),
        input_metric=dp.absolute_distance(T=int),
        output_domain=dp.atom_domain(T=str),
        output_metric=dp.discrete_distance(),
        function=lambda x: str(x + 10_000),
        stability_map=lambda x: x)

    sc_odo2 = sc_odo >> postprocessor
