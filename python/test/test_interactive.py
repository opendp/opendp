...
def test_sequential_odometer():
    ...
    m_sum = o_sc.input_space >> dp.t.then_clamp((0, 10)) >> dp.t.then_sum() >> dp.m.then_laplace(100.)
    ...
    t_sum = o_sc.input_space >> dp.t.then_clamp((0, 10)) >> dp.t.then_sum()
    m_sum_compositor = t_sum >> dp.c.then_sequential_composition(
        output_measure=dp.max_divergence(),
        d_in=t_sum.map(max_influence),
        d_mids=[0.2, 0.09]
    )
    qbl_summed = oqbl_sc(m_sum_compositor)
