...
def test_cast_impute():
    ...
    assert caster([1.0, 2.0, 3.0]) == [1, 2, 3]

    caster = dp.t.make_cast(*input_space, TOA=int) >> dp.t.then_impute_constant(1)
    assert caster([float("nan"), 2.0]) == [1, 2]
...
def test_cast_default():
    caster = dp.t.make_cast_default(
        dp.vector_domain(dp.atom_domain(T=float)), dp.symmetric_distance(), TOA=int
    )
    assert caster([float("nan"), 2.0]) == [0, 2]
...
def test_is_null():
    ...
    tester = dp.t.make_split_lines() >> dp.t.then_cast(TOA=float) >> dp.t.then_is_null()
    assert tester("nan\n1.\ninf") == [True, False, False]
...
def test_inherent_cast__impute():
    casted = (
        dp.t.make_split_lines()
        >> dp.t.then_cast_inherent(TOA=float)
        >> dp.t.then_impute_constant(constant=9.0)
    )
...
def test_inherent_cast__impute_uniform():
    casted = (
        dp.t.make_split_lines()
        >> dp.t.then_cast_inherent(TOA=float)
        >> dp.t.then_impute_uniform_float(bounds=(23.0, 32.5))
    )

    res = casted("a\n23.23\n12")
    assert res[1:] == [23.23, 12.0]
    assert 23.0 <= res[0] <= 32.5
    assert casted.check(1, 1)
...
def test_bounded_mean():
    query = dp.t.make_mean(
        dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=9),
        dp.symmetric_distance(),
    )
    assert query(FLOAT_DATA) == 5.0
    assert query.check(2, 10.0 / 9.0 + 1e-6)
...
def test_bounded_sum():
    ...
    query = dp.t.make_sum(
        dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0))), dp.symmetric_distance()
    )
    assert query(FLOAT_DATA) == 45.0
    # TODO: tighten the check
    assert query.check(1, 20.0)

    query = dp.t.make_sum(
        dp.vector_domain(dp.atom_domain(bounds=(0, 10))), dp.symmetric_distance()
    )
...
def test_sized_bounded_sum():
    ...
    domain = dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=9)
    metric = dp.symmetric_distance()
    query = (domain, metric) >> dp.t.then_sum()
    assert query(FLOAT_DATA) == 45.0
    assert query.check(1, 10.0 + 1e-12)

    domain = dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=10_000)
    query = (domain, metric) >> dp.t.then_sum()
    assert query.check(1, 10.0 + 1e-9)

    domain = dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=100_000)
    query = (domain, metric) >> dp.t.then_sum()
    assert query.check(1, 10.0 + 1e-8)

    domain = dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=1_000_000)
    query = (domain, metric) >> dp.t.then_sum()
    assert query.check(1, 10.0 + 1e-7)

    domain = dp.vector_domain(dp.atom_domain(bounds=(0.0, 10.0)), size=10_000_000)
    query = (domain, metric) >> dp.t.then_sum()
    assert query.check(1, 10.0 + 1e-5)
...
def test_transformation_check_debug():
    transformation = dp.t.make_count(
...
def test_count_distinct():
    transformation = dp.t.make_count_distinct(
...
def test_count_by_categories():
    input_space = dp.vector_domain(dp.atom_domain(T=str)), dp.symmetric_distance()
    query = dp.t.make_count_by_categories(
        *input_space, categories=["1", "3", "4"], MO=dp.L1Distance[int]
    )
...
def test_indexing():
    ...
    find = (
        input_space
        >> dp.t.then_find(categories=["1", "3", "4"])
    ...
    indexer = find.output_space >> dp.t.then_index(
        categories=["A", "B", "C"], null="NA"
    )
    assert indexer([0, 1, 3, 1, 5]) == ["A", "B", "NA", "B", "NA"]

    assert (find >> indexer)(STR_DATA) == [
        "A",
        "NA",
        "B",
        "C",
    ...
    assert (binner >> indexer)(INT_DATA) == [
        "A",
        "B",
        "C",
        "C",
...
def test_lipschitz_mul_float():
    trans = dp.t.make_sized_bounded_float_ordered_sum(
        10, (0.0, 10.0)
    ) >> dp.t.then_lipschitz_float_mul(1 / 10, (-3.0, 4.0))

    assert trans([3.0] * 10) == 0.4
...
def test_lipschitz_b_ary_tree():
    ...
    meas_quantiles = meas_base >> dp.t.make_quantiles_from_counts(
        bin_edges=[0.0, 10.0, 13.0, 17.0, 26.0, 70.0, 84.0, 100.0],
