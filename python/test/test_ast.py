...
def check_directive_order(docstring):
    ...
    unknown_directives = set(directives) - {
        ":param",
        ":rtype:",
        ":type",
        ":raises",
...
class Checker:
    def __init__(self, name, tree, docstring, is_public, is_verbose):
        ...
        args = self.tree.args
        self.all_ast_args = args.posonlyargs + args.args + args.kwonlyargs
    ...
    def _check_params(self):
        ...
        if self.is_public:
            ...
            if k_missing_v:
                self.errors.append(
                    f"params missing descriptions: {', '.join(k_missing_v)}"
                )
    ...
    def _check_types(self):
        ...
        ast_arg_annotation_dict = {
            arg.arg: getattr(arg, "annotation", None) for arg in self.all_ast_args
        }
        ast_type_dict = {
        ...
        if rtype_match:
            ...
            # If it's a single character, probably a typevar, which we won't try to resolve.
                ...
                if doc_rtype != ast_rtype:
                    self.errors.append(
                        f'update :rtype: from "{doc_rtype}" to "{ast_rtype}"'
                    )
