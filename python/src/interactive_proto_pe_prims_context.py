...
def error_eval(query, state):
    raise Exception(f"Unexpected system query {query}")

class Queryable:

    def __init__(self, state, eval, sys_eval=None):
        self.state = state
        self.eval = eval  # fn: T x Q x S -> A x S
        self.sys_eval = sys_eval or error_eval
    ...
    def _system_query(self, query):
        answer, new_state = self.sys_eval(query, self.state)
        self.state = new_state
        return answer


TreeContext = collections.namedtuple("TreeContext", ["parent", "index"])
ChildDidChangeQuery = collections.namedtuple("ChildDidChangeQuery", ["index", "change"])


def make_naive_leaf_queryable(state, inner_eval, context):
    # The structure of stuff we store in state
    State = collections.namedtuple("State", ["parent", "index", "inner_state"])
    state = State(parent=context.parent, index=context.index, inner_state=state)

    def eval(tag, query, state):
        answer, new_inner_state, change = inner_eval(query, state.inner_state, context)  # Raises exception on failure
        if state.parent is not None:
            parent_query = ChildDidChangeQuery(index=state.index, change=change)
            _ignored = state.parent._sys_query(parent_query)  # Raises exception on failure
