...
class Queryable:
    ...
    def __init__(self, state, _evaluate):
        self.state = state
        self._evaluate = _evaluate

    def query(self, query):
        answer, state = self._evaluate(query, self.state)
        self.state = state
        return answer

    def query1(self, query):
        answer = self.query(query)
        return answer.query(None)


class FixedQueryable(Queryable):

    def __init__(self, answer):
        def _evaluate(_query, state):
            return state
        super().__init__(answer, _evaluate)


class NestableQueryable:
    ...
    def query(self, query):
        ...
        if isinstance(answer, NestableQueryable):
            answer.parent = self
...
def make_concurrent_odometer():
    ...
    def function(data):
        ...
        return NestableQueryable(state, provision_for_self, execute_for_self, provision_for_child)
...
def make_sequential_odometer():
    ...
    def function(data):
        ...
        return NestableQueryable(state, execute_for_self, provision_for_self, provision_for_child)
