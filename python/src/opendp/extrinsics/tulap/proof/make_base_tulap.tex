...
def make_base_tulap(b: float, scale: float):
    input_domain = AllDomain(float)
    input_metric = AbsoluteDistance(float)
    output_measure = SmoothedMaxDivergence(float)
...
\end{tcolorbox}
    ...
    \begin{restatable}{prop}{propsamplingCND}\label{prop1}
      Let $f$ be a symmetric nontrivial tradeoff function and let $F_f$ be as in Definition \ref{def:CNDsynthetic}. Then the quantile function $F_f^{-1}:(0,1)\rightarrow \RR$ for $F_f$ can be expressed as
      \[F_f^{-1}(u) = \begin{cases}
      F_f^{-1}(1-f(u))-1&u<c\\
