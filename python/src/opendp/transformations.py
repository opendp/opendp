...
def make_lipschitz_sized_proportion_ci_mean(
    ...
    * Output Metric:  `AbsoluteDistance<TOA>`
    ...
    # Convert arguments to c types.
    c_strat_sizes = py_to_c(strat_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_sample_sizes = py_to_c(sample_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_TIA = py_to_c(TIA, c_type=ctypes.c_char_p)
    c_TOA = py_to_c(TOA, c_type=ctypes.c_char_p)
    
    # Call library function.
    lib_function = lib.opendp_transformations__make_lipschitz_sized_proportion_ci_mean
    lib_function.argtypes = [AnyObjectPtr, AnyObjectPtr, ctypes.c_char_p, ctypes.c_char_p]
    lib_function.restype = FfiResult
    
    output = c_to_py(unwrap(lib_function(c_strat_sizes, c_sample_sizes, c_TIA, c_TOA), Transformation))
...
def make_lipschitz_sized_proportion_ci_variance(
    ...
    * Output Metric:  `AbsoluteDistance<TOA>`
    ...
    c_strat_sizes = py_to_c(strat_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_sample_sizes = py_to_c(sample_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_mean_scale = py_to_c(mean_scale, c_type=AnyObjectPtr, type_name=TOA)
    c_TIA = py_to_c(TIA, c_type=ctypes.c_char_p)
    c_TOA = py_to_c(TOA, c_type=ctypes.c_char_p)
    
    # Call library function.
    lib_function = lib.opendp_transformations__make_lipschitz_sized_proportion_ci_variance
    lib_function.argtypes = [AnyObjectPtr, AnyObjectPtr, AnyObjectPtr, ctypes.c_char_p, ctypes.c_char_p]
    lib_function.restype = FfiResult
    
    output = c_to_py(unwrap(lib_function(c_strat_sizes, c_sample_sizes, c_mean_scale, c_TIA, c_TOA), Transformation))
...
def make_postprocess_proportion_ci(
    ...
    * Output Domain:  `VectorDomain<AllDomain<TA>>`
    ...
    # Convert arguments to c types.
    c_strat_sizes = py_to_c(strat_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_sum_scale = py_to_c(sum_scale, c_type=ctypes.c_void_p, type_name=TA)
    c_size_scale = py_to_c(size_scale, c_type=ctypes.c_void_p, type_name=TA)
    c_TA = py_to_c(TA, c_type=ctypes.c_char_p)
    
    # Call library function.
    lib_function = lib.opendp_transformations__make_postprocess_proportion_ci
    lib_function.argtypes = [AnyObjectPtr, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_char_p]
    lib_function.restype = FfiResult
    
    output = c_to_py(unwrap(lib_function(c_strat_sizes, c_sum_scale, c_size_scale, c_TA), Transformation))
...
def make_postprocess_sized_proportion_ci(
    ...
    * Output Domain:  `VectorDomain<AllDomain<TA>>`
    ...
    c_strat_sizes = py_to_c(strat_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_sample_sizes = py_to_c(sample_sizes, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Vec', args=[usize]))
    c_scale = py_to_c(scale, c_type=ctypes.c_void_p, type_name=TA)
    c_TA = py_to_c(TA, c_type=ctypes.c_char_p)
    
    # Call library function.
    lib_function = lib.opendp_transformations__make_postprocess_sized_proportion_ci
    lib_function.argtypes = [AnyObjectPtr, AnyObjectPtr, ctypes.c_void_p, ctypes.c_char_p]
    lib_function.restype = FfiResult
    
    output = c_to_py(unwrap(lib_function(c_strat_sizes, c_sample_sizes, c_scale, c_TA), Transformation))
