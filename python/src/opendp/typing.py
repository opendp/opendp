...
class RuntimeType(object):
    ...
    def __init__(self, origin, args=None):
        if not isinstance(origin, str):
            raise ValueError("origin must be a string", origin)  # pragma: ignore
        self.origin = origin
        self.args = args or []
    ...
    def parse(cls, type_name: RuntimeTypeDescriptor, generics: Optional[list[str]] = None) -> Union["RuntimeType", str]:
        ...
        if type_name == tuple:
            raise UnknownTypeException("non-parameterized argument")  # pragma: ignore
        ...
        if np is not None and isinstance(public_example, np.ndarray):
            ...
            if public_example.ndim == 1:
                ...
                if inner_type is None:
                    raise UnknownTypeException(f"Unknown numpy array dtype: {public_example.dtype.type}")  # pragma: ignore
                return RuntimeType('Vec', [inner_type])

            raise UnknownTypeException("arrays with greater than one axis are not yet supported")  # pragma: ignore
    ...
    def parse_or_infer(
        ...
        raise UnknownTypeException("either type_name or public_example must be passed")  # pragma: ignore
...
class GenericType(RuntimeType):
    def __repr__(self):
        raise UnknownTypeException(f"attempted to create a type_name with an unknown generic: {self.origin}")  # pragma: ignore
