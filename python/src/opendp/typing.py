...
class RuntimeType(object):
    ...
    def infer(cls, public_example: Any, py_object=False) -> Union["RuntimeType", str]:
        ...
        beam_type_name = _try_infer_beam(public_example)
...
def _try_infer_beam(public_example: Any) -> Optional[RuntimeType]:
    """
    Try to infer the RuntimeType of a beam object.
    Fails gracefully if user hasn't installed beam.

    This is implemented as a separate function, rather than as a method on RuntimeType,
    to isolate dependencies on beam. If we stick with this approach to external compute,
    we can fold it in later.
    """
    try:
        import apache_beam
    except ImportError:
    ...
    if isinstance(public_example, apache_beam.PCollection):
        ...
        # In some circumstances, beam can't figure out the element_type of the PCollection.
        # We use explicit annotations, so this should never happen, but handle it just in case.
        if isinstance(element_type, apache_beam.typehints.typehints.AnyTypeConstraint):
            inner_type = UnknownType("apache_beam.typehints.typehints.AnyTypeConstraint")
        else:
