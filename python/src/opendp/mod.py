...
class Queryable(object):
    ...
    def __call__(self, query):
        from opendp.core import queryable_eval, odometer_queryable_invoke_measurement, odometer_queryable_invoke_odometer
        
        if self.query_type == "AnyOdometerQuery":
            if isinstance(query, Measurement):
                return odometer_queryable_invoke_measurement(self.value, query)
            if isinstance(query, Odometer):
                return odometer_queryable_invoke_odometer(self.value, query)
