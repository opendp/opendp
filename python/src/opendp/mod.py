...
class Measurement(ctypes.POINTER(AnyMeasurement)): # type: ignore[misc]
    ...
    def __rshift__(self, other: Union["Function", "Transformation", Callable]) -> "Measurement":
        ...
        if not isinstance(other, Function):
            if not callable(other):
                raise ValueError(f'Expected a callable instead of {other}')  # pragma: ignore
    ...
    def __iter__(self):
        ...
        raise ValueError("Measurement does not support iteration")  # pragma: ignore
...
class Transformation(ctypes.POINTER(AnyTransformation)): # type: ignore[misc]
    ...
    def __rshift__(self, other: Union["Measurement", "Transformation", "PartialConstructor"]) -> Union["Measurement", "Transformation", "PartialConstructor", "PartialChain"]:  # type: ignore[name-defined] # noqa F821
        ...
        raise ValueError(f"rshift expected a measurement or transformation, got {other}")  # pragma: ignore
    ...
    def __iter__(self):
        raise ValueError("Transformation does not support iteration")  # pragma: ignore
...
class Function(ctypes.POINTER(AnyFunction)): # type: ignore[misc]
    ...
    def __iter__(self):
        raise ValueError("Function does not support iteration")  # pragma: ignore
...
class Metric(ctypes.POINTER(AnyMetric)): # type: ignore[misc]
    ...
    def __iter__(self):
        raise ValueError("Metric does not support iteration")  # pragma: ignore
...
class Measure(ctypes.POINTER(AnyMeasure)): # type: ignore[misc]
    ...
    def __iter__(self):
        raise ValueError("Measure does not support iteration")  # pragma: ignore
...
class PartialConstructor(object):
    ...
    def __rrshift__(self, other):
        ...
        raise TypeError(f"Cannot chain {type(self)} with {type(other)}")  # pragma: ignore
...
def binary_search(
    ...
    if bounds is None:
        raise ValueError("unable to infer bounds")  # pragma: ignore

    if len(set(map(type, bounds))) != 1:
        raise TypeError("bounds must share the same type")  # pragma: ignore
    ...
    if isinstance(lower, float):
        ...
        raise TypeError("bounds must be either float or int")  # pragma: ignore
...
def exponential_bounds_search(
    ...
    if T is None:
        ...
        if check_type(0.):
            ...
            raise TypeError("unable to infer type `T`; pass the type `T` or bounds")  # pragma: ignore
    ...
    def signed_band_search(center, at_center, sign):
        ...
        if T == int:
            ...
            raise TypeError(f"unknown type {T}. Must be one of int, float")  # pragma: ignore
