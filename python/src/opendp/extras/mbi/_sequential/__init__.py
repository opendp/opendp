...
from opendp.extras.mbi._utilities import Algorithm, get_associated_metric, get_cardinalities
...
class Sequential(Algorithm):
    ...
    def __post_init__(self):
        ...
        if any(not isinstance(a, Algorithm) for a in self.algorithms):
            raise ValueError("algorithms must be instances of Algorithm")
    ...
    @property
    def make(self):
        return make_sequential_marginals


def make_sequential_marginals(
    ...
    *,
    ...
    weights = np.array(algorithm.weights or [1] * len(algorithm.algorithms))
    ...
    def function(
        ...
        current_model = model
        for algo, d_mid in zip(algorithm.algorithms, d_mids):
            m_algo = algo.make(
                input_domain,
                input_metric,
                output_measure,
