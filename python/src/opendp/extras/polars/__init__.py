...
from opendp.domains import series_domain, lazyframe_domain, option_domain, atom_domain, categorical_domain, unknown_value_domain
...
class DPExpr(object):
    ...
    def mean(self, bounds: Tuple[float, float], scale: tuple[float, float] | None = None, cast: bool = True):
        ...
        >>> print(expression)
        [(col("numbers").cast(...).fill_null(...).clip(...).sum()...:noise(...)) / (col("numbers").len()...:noise(...))]
        """
