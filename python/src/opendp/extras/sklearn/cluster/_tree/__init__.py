...
def _make_split(input_domain, input_metric, *, splitter: Quadtree | LSH, depth: int):
    return _make_transformation(
        ...
        function=lambda data: _split(data, splitter, depth),
...
def _budget_without_slack(budget: float) -> float:
    np = _np()
    if budget <= 0.0:
        raise ValueError("budget must be positive")
    return float(np.nextafter(float(budget), 0.0))
...
def _cell_l1_diameter(splitter: Quadtree) -> float:
    np = _np()
    side_lengths = np.asarray(splitter.upper, dtype=float) - np.asarray(splitter.lower, dtype=float)
    return float(np.sum(side_lengths))
...
def _kmedians_axis_scores(data: "numpy.ndarray", lo: "numpy.ndarray", hi: "numpy.ndarray"):
    ...
    mids = 0.5 * (lo + hi)
    scores = np.zeros(d, dtype=float)
    parent_median = np.median(data, axis=0)
    parent_cost = float(np.abs(data - parent_median).sum())
    ...
    for axis in range(d):
        ...
        if len(left):
            ...
            left_cost = float(np.abs(left - left_median).sum())

        right_cost = 0.0
        if len(right):
            right_median = np.median(right, axis=0)
            right_cost = float(np.abs(right - right_median).sum())

        # Normalized split gain in the k-medians objective:
        #   (L1(parent) - L1(left) - L1(right)) / n
        scores[axis] = (parent_cost - left_cost - right_cost) / float(n)
    return scores
...
def _axis_score_sensitivity(splitter: Quadtree, objective: Literal["kmeans", "kmedians"]) -> float:
    ...
    if objective == "kmedians":
        # The normalized L1 split gain satisfies
        #   0 <= score_a <= sum_j (upper_j - lower_j)
        # for every axis a, because the per-point parent k-medians cost is
        # bounded by the cell's L1 diameter and child costs are nonnegative.
        # Under add/remove adjacency, the L_infinity sensitivity is bounded by
        # the same public quantity.
        return _cell_l1_diameter(splitter)
    raise ValueError("objective must be 'kmeans' or 'kmedians'")
