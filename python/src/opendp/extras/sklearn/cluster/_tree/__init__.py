...
class Node:
    ...
    # Axis used to split the parent cell into this node and its sibling.
...
class Quadtree:
    lower: "numpy.ndarray"
    upper: "numpy.ndarray"
    # Optional override for the split axis at the current node.
    axis: int | None = None
...
def make_private_tree(
    ...
    *,
    ...
    When `splitter` is a quadtree and `select_branch_fraction > 0`, reserve a
    fraction of the current level budget to privately choose the split axis via
    `make_noisy_max` over objective-aligned split scores.
    ...
    :param select_branch_fraction: Fraction of the current level budget reserved for axis selection.
    :param select_branch_max_depth: If set, only select axes privately up to this depth (inclusive of root at depth 0).
    """
    np = _np()
    level_allocation = _normalize(level_allocation)

    selection_enabled = (
        isinstance(splitter, Quadtree)
        and splitter.axis is None
        and select_branch_fraction > 0.0
        and (select_branch_max_depth is None or depth < select_branch_max_depth)
    )
    if not selection_enabled:
        return _make_private_tree_fixed(
            input_domain,
            input_metric,
            output_measure,
            splitter=splitter,
            ...
            objective=objective,
            depth=depth,
            select_branch_fraction=select_branch_fraction,
            select_branch_max_depth=select_branch_max_depth,
        )

    quad_splitter = cast(Quadtree, splitter)
    total_budget = _privacy_budget_for_noise_scale(output_measure, scale)
    current_level_budget = float(level_allocation[0]) * total_budget
    selection_budget = float(select_branch_fraction) * current_level_budget
    if not (0.0 < selection_budget < current_level_budget):
        raise ValueError("select_branch_fraction must be in (0, 1) when branch selection is enabled")

    remaining_head_budget = current_level_budget - selection_budget
    tail_budget = float(level_allocation[1:].sum()) * total_budget
    remaining_budgets = np.concatenate([[remaining_head_budget], level_allocation[1:] * total_budget])
    remaining_total_budget = float(remaining_budgets.sum())
    remaining_allocation = remaining_budgets / remaining_total_budget
    remaining_scale = _noise_scale_for_budget(output_measure, remaining_total_budget)

    selection_sensitivity = _axis_score_sensitivity(quad_splitter, objective)
    selection_scale = _noisy_max_scale_for_budget(
        output_measure,
        selection_budget,
        sensitivity=selection_sensitivity,
    )
    next_local_scale = None
    if len(remaining_allocation) > 1:
        child_allocation = _normalize(remaining_allocation[1:])
        child_selection_enabled = (
            select_branch_fraction > 0.0
            and (select_branch_max_depth is None or depth + 1 < select_branch_max_depth)
        )
        next_local_budget = _effective_count_budget_for_level(
            float(child_allocation[0]) * tail_budget,
            selection_enabled=child_selection_enabled,
            select_branch_fraction=select_branch_fraction,
        )
        next_local_scale = _noise_scale_for_budget(output_measure, next_local_budget)

    prototype_followup = _make_private_tree_fixed(
        input_domain,
        input_metric,
        output_measure,
        splitter=Quadtree(quad_splitter.lower, quad_splitter.upper, axis=0),
        level_allocation=remaining_allocation,
    ...
    followup_budget = _budget_with_slack(prototype_followup.map(1))

    m_select_axis = _make_private_split_axis_selector(
        input_domain,
        input_metric,
        output_measure,
        splitter=quad_splitter,
        scale=selection_scale,
        objective=objective,
        next_local_scale=next_local_scale,
    )
    ...
    comp = dp.c.make_adaptive_composition(
        ...
        d_in=1,
        d_mids=[_budget_with_slack(m_select_axis.map(1)), followup_budget],
    )

    def postprocess(qbl):
        axis = qbl(m_select_axis)
        selected_splitter = Quadtree(quad_splitter.lower, quad_splitter.upper, axis=int(axis))
        followup = _make_private_tree_fixed(
            input_domain,
            input_metric,
            output_measure,
            splitter=selected_splitter,
            level_allocation=remaining_allocation,
            scale=remaining_scale,
            objective=objective,
            depth=depth,
...
def _make_private_tree_fixed(
    ...
    *,
    ...
    level_allocation,
    scale: float,
    objective: Literal["kmeans", "kmedians"],
    depth: int,
    select_branch_fraction: float,
    select_branch_max_depth: int | None,
):
    np = _np()
    ...
    split_axis = _split_axis(splitter, depth) if isinstance(splitter, Quadtree) else None

    local_budget = float(head_allocation) * total_budget
    ...
    m_child_counts = _make_private_child_counts(
        ...
        lambda nodes: [Node(node.value, node.var, split_axis=split_axis) for node in nodes],
        TO="ExtrinsicObject",
    )
    ...
    tail_budget = float(tail_allocation.sum()) * total_budget
    ...
    child_local_scale = _noise_scale_for_budget(
        output_measure,
        _effective_count_budget_for_level(
            float(child_allocation[0]) * tail_budget,
            selection_enabled=(
                isinstance(splitter, Quadtree)
                and select_branch_fraction > 0.0
    ...
    def postprocess(qbl):
        ...
        for count_node, recurse, followup in zip(count_nodes, recurse_flags, followup_outputs):
            if recurse:
                out.append(Node(count_node.value, count_node.var, followup, split_axis=split_axis))
            else:
                ...
                out.append(Node(value, var, split_axis=split_axis))
        return out
...
def _make_private_child_counts(input_domain, input_metric, output_measure, *, scale: float):
    ...
    trans = _make_transformation(
        input_domain=input_domain,
        input_metric=input_metric,
        output_domain=dp.vector_domain(dp.atom_domain(T=dp.i32), size=2),
...
def _make_private_split_axis_selector(
    input_domain,
    input_metric,
    output_measure,
    *,
    ...
    scale: float,
    objective: Literal["kmeans", "kmedians"],
    next_local_scale: float | None,
):
    d = len(splitter.lower)
    score_trans = _make_axis_score_vector(
        input_domain,
        input_metric,
        splitter=splitter,
        objective=objective,
    ...
    return score_trans >> dp.m.make_noisy_max(
        input_domain=dp.vector_domain(dp.atom_domain(T=float, nan=False), size=d),
...
def _make_axis_score_vector(
    ...
    *,
    ...
    np = _np()
    d = len(splitter.lower)
    sensitivity = _axis_score_sensitivity(splitter, objective)
    ...
    return _make_transformation(
        input_domain=input_domain,
        input_metric=input_metric,
        output_domain=dp.vector_domain(dp.atom_domain(T=float, nan=False), size=d),
        output_metric=dp.linf_distance(T=float),
        function=function,
        stability_map=lambda d_in: float(d_in) * sensitivity,
    )
...
def _split_axis(splitter: Quadtree, depth: int) -> int:
    if splitter.axis is not None:
        return int(splitter.axis)
    return int(depth % len(splitter.lower))



def _split_axis_mid(splitter: Quadtree, depth: int):
    np = _np()
    axis = _split_axis(splitter, depth)
...
def _noisy_max_scale_for_budget(
    output_measure, budget: float, *, sensitivity: float
) -> float:
    np = _np()
    if budget <= 0:
        raise ValueError("budget must be positive")
    ...
    # For sensitivity 1 under L_infinity, make_noisy_max with scale=1 has:
    # - epsilon = 2 under MaxDivergence
    # - rho = 1/8 under ZeroConcentratedDivergence
    # For general sensitivity s, privacy loss scales linearly with s for
    # MaxDivergence and quadratically with s for ZeroConcentratedDivergence.
    if output_measure == dp.max_divergence():
        return _scale_with_slack(2.0 * sensitivity / budget)
    if output_measure == dp.zero_concentrated_divergence():
        return _scale_with_slack(sensitivity * np.sqrt(1.0 / (8.0 * budget)))
    raise ValueError("output_measure must be MaxDivergence or ZeroConcentratedDivergence")
...
def _effective_count_budget_for_level(
    local_budget: float,
    *,
    ...
    if selection_enabled:
        return (1.0 - float(select_branch_fraction)) * float(local_budget)
    return float(local_budget)
...
def _axis_scores(
    ...
    *,
    ...
    if objective == "kmeans":
        scores = _kmeans_axis_scores(data, lo, hi)
    elif objective == "kmedians":
        scores = _kmedians_axis_scores(data, lo, hi)
    else:
        raise ValueError("objective must be 'kmeans' or 'kmedians'")

    if next_local_scale is None:
        return scores

    next_noise_std = np.sqrt(_noise_var(output_measure, next_local_scale))
    penalties = np.empty(d, dtype=float)
    for axis in range(d):
        child_splitters = _child_splitters(Quadtree(lo, hi, axis=axis), 0)
        penalties[axis] = 0.5 * sum(_leaf_penalty(next_noise_std, child, objective) for child in child_splitters)
    return scores - penalties



def _kmeans_axis_scores(data: "numpy.ndarray", lo: "numpy.ndarray", hi: "numpy.ndarray"):
    np = _np()
    n, d = data.shape
    if n <= 1:
        return np.zeros(d, dtype=float)
    ...
    for axis in range(d):
        ...
        # Normalized split gain in SSE:
        #   (SSE(parent) - SSE(left) - SSE(right)) / n
        # = (n_left * n_right / n^2) * ||mu_left - mu_right||^2
        scores[axis] = (float(n_left) * float(n_right) / float(n * n)) * float(diff @ diff)
    return scores



def _kmedians_axis_scores(data: "numpy.ndarray", lo: "numpy.ndarray", hi: "numpy.ndarray"):
    np = _np()
    n, d = data.shape
    parent = Quadtree(lo, hi)
    current_penalty = _leaf_penalty(n, parent, "kmedians")
    mids = 0.5 * (lo + hi)
    scores = np.zeros(d, dtype=float)
    ...
    for axis in range(d):
        ...
        n_right = n - n_left
        child_splitters = _child_splitters(Quadtree(lo, hi, axis=axis), 0)
        next_penalty = _leaf_penalty(n_left, child_splitters[0], "kmedians")
        next_penalty += _leaf_penalty(n_right, child_splitters[1], "kmedians")
        scores[axis] = current_penalty - next_penalty
    return scores



def _axis_score_sensitivity(splitter: Quadtree, objective: Literal["kmeans", "kmedians"]) -> float:
    diam = _cell_diameter(splitter)
    if objective == "kmeans":
        # The normalized SSE gain satisfies
        #   0 <= score_a <= (1/4) * diam(cell)^2
        # for every axis a, so under add/remove adjacency the L_infinity
        # sensitivity is bounded by the same public quantity.
        return 0.25 * diam * diam
    if objective == "kmedians":
        return 4.0 * diam
    raise ValueError("objective must be 'kmeans' or 'kmedians'")
