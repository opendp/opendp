...
def make_private_eigenvector(
    ...
    if input_desc.p != 2:
        raise ValueError("input_domain must have bounded L2-norm")  # pragma: ignore

    if input_metric != dp.symmetric_distance():
        raise ValueError("expected symmetric distance input metric")  # pragma: ignore
...
def make_private_eigenvectors(
    ...
    input_desc = input_domain.descriptor
    if input_desc.p != 2:
        raise ValueError("input_domain must have bounded L2 row norm")  # pragma: ignore
