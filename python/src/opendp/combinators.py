...
def make_basic_composition(
    ...
    * compositor: all privacy parameters specified up-front (via the map)
    ...
    # No type arguments to standardize.
    # Convert arguments to c types.
    c_measurements = py_to_c(measurements, c_type=FfiSlicePtr, type_name=RuntimeType(origin='Vec', args=[AnyMeasurementPtr]))
...
def make_chain_ot(
    ...
    r"""Construct the functional composition (`odometer1` ○ `transformation0`).
...
def make_chain_po(
    ...
    r"""Construct the functional composition (`function1` ○ `odometer0`).
...
def make_sequential_odometer(
    ...
    r"""Construct a sequential odometer queryable that interactively composes odometers or interactive measurements.
