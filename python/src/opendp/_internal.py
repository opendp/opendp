...
def _binary_search(
    ...
    :param lower: Optional lower bound on the input to ``predicate``.
    :param upper: Optional upper bound on the input to ``predicate``.
    ...
    T = RuntimeType.parse_or_infer(type_name=T, public_example=get_first_non_null(lower, upper))

    # Convert arguments to c types.
    c_predicate = py_to_c(predicate, c_type=CallbackFnPtr, type_name="bool")
    c_lower = py_to_c(lower, c_type=ctypes.c_void_p, type_name=RuntimeType(origin='Option', args=[T]))
    c_upper = py_to_c(upper, c_type=ctypes.c_void_p, type_name=RuntimeType(origin='Option', args=[T]))
    ...
    lib_function.argtypes = [CallbackFnPtr, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_bool, ctypes.c_char_p]
    lib_function.restype = FfiResult

    output = c_to_py(unwrap(lib_function(c_predicate, c_lower, c_upper, c_return_sign, c_T), AnyObjectPtr))
    try:
        output.__opendp_dict__ = {
            '__function__': '_binary_search',
            '__module__': 'internal',
            '__kwargs__': {
                'predicate': predicate, 'lower': lower, 'upper': upper, 'return_sign': return_sign, 'T': T
            },
        }
