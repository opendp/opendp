...
use crate::error::*;
use crate::traits::{CheckedAbs, DistanceConstant, InfCast, SaturatingAdd, CheckedMul, ExactIntCast, CheckNull};
...
pub fn make_bounded_sum<T>(
    ...
    Ok(Transformation::new(
        ...
        StabilityRelation::new_from_constant(
            lower.checked_abs().ok_or_else(|| err!(MakeTransformation))?.total_max(
                upper.checked_abs().ok_or_else(|| err!(MakeTransformation))?)?)))
}
