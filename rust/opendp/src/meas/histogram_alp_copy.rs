// This is an alternative hashing approach that sidesteps the built-in hasher but is more complicated.
...
// use crate::samplers::{CastInternalReal, fill_bytes, SampleBernoulli};
...
// pub trait AlpHasher: Hasher + Sized + Clone {
//     fn new(l: u32) -> Fallible<Self>;
//     fn hash<K: Hash>(&self, x: &K) -> usize {
...
// }
// #[derive(Clone)]
// pub struct DefaultAlpHasher {
//     a: u64,
//     b: u64,
//     l: u32
// }
// impl AlpHasher for DefaultAlpHasher {
//     fn new(l: u32) -> Fallible<Self> {
//         let mut buf = [0u8; 8];
//         fill_bytes(&mut buf)?;
...
//         Ok(Self {a: a | 1u64, b, l})
//     }
// }
//
// // for testing, construct a hasher with fixed a and b
// #[cfg(test)]
// impl DefaultAlpHasher { fn new_debug(a: u64, b: u64, l: u32) -> Self { Self {a: a | 1u64, b, l} } }
//
// impl Hasher for DefaultAlpHasher {
//     fn finish(&self) -> u64 {
//         self.a
//     }
//
//     fn write(&mut self, bytes: &[u8]) {
//         let DefaultAlpHasher { a, b, l } = *self;
//         let mut buffer = [0u8; 8];
//
//         let a = bytes.chunks(8).map(|v| {
//             buffer[..v.len()].copy_from_slice(v);
//             u64::from_be_bytes(buffer)
//         })
//             .fold(a, |a, x| x.wrapping_mul(a).wrapping_add(b) >> (64 - l) | 1u64);
//         *self = DefaultAlpHasher {a, b, l};
//     }
// }
//
// fn sample_hash_function<K, H: 'static + AlpHasher>(l: u32) -> Fallible<Rc<dyn Fn(&K) -> usize>>
//     where K: Hash {
//     let hasher = H::new(l)?;
//     Ok(Rc::new(move |x: &K| hasher.hash(x) as usize))
// }
//
...
// }
//
// pub fn make_alp_histogram_parameterized<K, C, T, H: 'static + AlpHasher>(n: usize, alpha: T, scale: T, beta: C, size_factor: u32)
//                                                                          -> Fallible<Measurement<SizedHistogramDomain<K, C>,
//                                                                              AlpDomain<K, T>,
//                                                                              L1Distance<C>, MaxDivergence<T>>>
...
//     let h = (0..m).map(|_| sample_hash_function::<K, H>(exp)).collect::<Fallible<HashFunctions<K>>>()?;
//
...
//           T: 'static + num::Float + DistanceCast + CastInternalReal {
//
//     make_alp_histogram_parameterized::<K, C, T, DefaultAlpHasher>(n, T::from(ALPHA_DEFAULT).unwrap(), scale, beta, SIZE_FACTOR_DEFAULT)
// }
//
...
//         assert_eq!(DefaultAlpHasher::new_debug(4, 5, 64).hash(&3), 17);
//         assert_eq!(DefaultAlpHasher::new_debug(4, 5, 63).hash(&3), 8);
//
//         assert_eq!(DefaultAlpHasher::new_debug(u64::MAX, 0, 2).hash(&1), 3);
//         assert_eq!(DefaultAlpHasher::new_debug(u64::MAX, 0, 3).hash(&1), 7);
//
//         assert_eq!(DefaultAlpHasher::new_debug(u64::MAX, 0, 16).hash(&3), (1 << 16) - 1);
//
//         Ok(())
//     }
//
...
//         let h = sample_hash_function::<_, DefaultAlpHasher>(5)?;
//
//         for i in 0u64..20u64 {
//             assert!(h(&i) < (1 << 5));
//         }
