...
use crate::core::{PrivacyRelation, Measurement, SensitivityMetric};
use crate::dist::SmoothedMaxDivergence;
use statrs::function::erf;
use super::ADDITIVE_GAUSS_CONST;
use crate::traits::{InfCast, CheckNull};
use crate::error::*;
use crate::meas::GaussianDomain;
use crate::samplers::SampleGaussian;


pub fn make_base_analytic_gaussian<D>(scale: D::Atom) -> Fallible<Measurement<D, D, D::Metric, SmoothedMaxDivergence<D::Atom>>>
    ...
    Ok(Measurement::new(
        ...
        make_analytic_gaussian_privacy_relation(scale),
    ))
}

fn make_analytic_gaussian_privacy_relation<T, MI>(
    scale: T
) -> PrivacyRelation<MI, SmoothedMaxDivergence<T>>
    where T: 'static + Clone + Float,
          f64: InfCast<T>,
    ...
    PrivacyRelation::new_fallible(move |&d_in: &T, &(eps, del): &(T, T)| {
        ...
        if eps.is_sign_negative() {
            return fallible!(InvalidDistance, "analytic gaussian mechanism: epsilon must not be negative")
        }
        ...
        Ok(eps.min(1.) >= (d_in / scale) * (ADDITIVE_GAUSS_CONST + 2. * del.recip().ln()).sqrt())
    })
}
