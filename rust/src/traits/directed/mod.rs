...
///
/// let sum = CInterval::between(1.0, 2.0)?
///     .add(CInterval::between(3.0, 4.0)?)?;
///
/// assert!(sum.lower_f64()? <= 4.0);
/// assert!(sum.upper_f64()? >= 6.0);
/// # Ok::<(), opendp::error::Error>(())
/// ```
#[derive(Clone)]
    ...
    Bk: IntervalBackend,
{
    ...
    pub fn clamp(self, min: f64, max: f64) -> Fallible<Self> {
        if min > max {
            return fallible!(
                FailedFunction,
                "invalid clamp bounds: minimum exceeds maximum"
            );
        }
