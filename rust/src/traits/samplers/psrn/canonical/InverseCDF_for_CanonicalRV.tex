...
the algorithm returns \texttt{Ok(Some(out))} where \texttt{out} is the inverse cumulative distribution function of \texttt{CanonicalRV} (which includes a rescale and shift) evaluated at \texttt{uniform}
with error in direction \texttt{R}, returns \texttt{Ok(None)} if the quantile function returns \texttt{None},
or returns an error if evaluating the tradeoff function fails.
...
If evaluating \texttt{self.tradeoff} fails while computing \texttt{f\_inv},
then the implementation propagates that error.
If \texttt{quantile\_cnd} returns \texttt{None},
the implementation returns \texttt{Ok(None)}.
