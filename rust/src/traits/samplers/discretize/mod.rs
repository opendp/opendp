...
pub trait CastInternalRational {
    /// # Proof Definition
    /// For any [`dashu::rational::RBig`] `v`, return `out`, the nearest representable value of type `Self`.
    ...
    /// `Ok(out)`, where `out` is a [`dashu::rational::RBig`] that exactly represents `self`.
