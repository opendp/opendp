...
\begin{proof} 
    $\texttt{x} \geq 0$ and \texttt{x} is of type \docsrs{dashu}{rational/struct}{Rbig}, so there exists some non-negative integer $s$ and positive integer $t$ such that $x = s/t$. 
    This is why \texttt{Rational.into\_numer\_denom} is infallible. 
    Since $t$ is a positive integer, the preconditions on \rustdoc{traits/samplers/uniform/fn}{sample_uniform_ubig_below} are met,  
...
\begin{lemma}\label{geom_1_t}\cite{CKS20} 
    \begin{itemize} 
        \item Let $u$ be a realization of a random variable $U \sim \textrm{Uniform}(0, t)$, supported on $[0, t)$. 
        \item Let $d$ be a realization of a random variable $D \sim \textrm{Bernoulli}(\exp(-u/t))$ 
        \item Let $v$ be a realization of a random variable $V \sim \textrm{Geometric}(1 - \exp(-1))$ 
    \end{itemize}  
...
\begin{proof} 
    ...
    \begin{align*} 
        ...
        &= P[U = u_z | D = \top] P[V = v_z] && \text{as $U$ and $V$ are independent, and $V$ and $D$ are independent}\\ 
...
\begin{proof} 
    As shown in \ref{err-e}, the preconditions for \rustdoc{traits/samplers/uniform/fn}{sample_uniform_ubig_below} on line \ref{line:U}, 
    ...
    By \ref{geom_1_t}, $\mathtt{z}$ is a realization of $Z \sim \textrm{Geometric}(1 - \exp(-1/t))$.  
    Since \texttt{z} is a realization of $Z \sim \textrm{Geometric}(1 - \exp(-1/t))$,  
    then by \ref{divide_geometric}, $\lfloor Z / s \rfloor$ is distributed as $\textrm{Geometric}(1 - \exp(-s/t))$. 
