...
\begin{theorem} \label{geom_s_t} \cite{CKS20} 
    Given any $s,t \in \mathbb{Z}_+$ $D \sim \textrm{Bernoulli}(\exp(-u/t))$, $Z \sim \textrm{Geometric}(1-\exp(-1/t))$,  
    define $Y = \lfloor Z / s \rfloor$. 
    Then $Y \sim \textrm{Geometric}(1 - \exp(-s/t))$. 
\end{theorem} 
...
\begin{proof} 
    ...
    then by \ref{geom_s_t}, \texttt{out} is distributed as $\textrm{Geometric}(1 - \exp(-x))$.     
\end{proof} 
