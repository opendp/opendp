...
\texttt{upper} of type \texttt{UBig} 
...
Let \texttt{range} denote the size of the sample space in this many bytes ($2^{(\texttt{byte\_len} \cdot 8)}$). 
Let \texttt{sample} be a uniform sample between $[0, \texttt{range})$ by flipping $\texttt{byte\_len} \cdot 8$ even coins. 
...
\begin{itemize} 
    ...
    \item the remaining trailing $(\texttt{range} \mod \texttt{upper})$ numbers 
\end{itemize} 
 
It is equivalent to only reject trailing numbers, 
and return the sample modulo \texttt{upper}. 
Since $\texttt{range} = \texttt{threshold} + (\texttt{range} \mod \texttt{upper})$, 
then $\texttt{threshold} = \texttt{range} - (\texttt{range} \mod \texttt{upper})$. 
