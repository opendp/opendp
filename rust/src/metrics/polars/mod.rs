...
pub fn id_sites_root_names(id_sites: &[Binding]) -> HashSet<polars::prelude::PlSmallStr> {
    id_sites
        .iter()
...
pub fn unique_id_expr(id_sites: &[Binding]) -> Fallible<Option<Expr>> {
    let sites = id_sites.iter().filter(|site| !site.exprs.is_empty()).fold(
        Vec::<Binding>::new(),
        |mut acc, site| {
...
pub fn filter_id_sites(id_sites: &[Binding], label: &str) -> Vec<Binding> {
    id_sites
        .iter()
