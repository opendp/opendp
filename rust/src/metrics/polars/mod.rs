...
/// Answers: which protected identifiers should this row be charged to?
///
/// Each factor in the vector is an expression that identifies exactly one
/// protected identifier in the selected `protect` space.
///
/// Semantics:
/// - one factor means one owner contribution source
/// - multiple factors mean multi-ownership
/// - a composite protected key is still one factor if packed into one `Expr`
/// - `vec![]` means this row currently carries no protected owner factors:
///   valid as metadata, identity under joins, but not enough for truncation
///
/// Distinct from `Binding`:
/// - `Binding`: this expression identifies exactly one entity in a semantic space
/// - `OwnerClaim`: this row should be charged to these protected identifiers
///
/// A row may have many exact-one identifiers available, but only some of them
/// are row owners for privacy accounting.
///
/// At the claim-set level:
/// - `vec![vec![]]` means there is a valid claim and that claim is empty
/// - `vec![]` means no valid ownership claim is available
pub type OwnerClaim = Vec<Expr>;

/// A frame-local alias/equality class for expressions that identify the same
/// semantic entity in a given space.
///
/// A `Binding` is not a cross-table schema object and it is not an ownership
/// declaration. It says:
///
/// - this expression identifies exactly one entity in a semantic space
///
/// After a join, expressions that are known equal in the resulting frame may
/// inhabit the same `Binding`.
...
fn normalize_exprs(exprs: impl IntoIterator<Item = Expr>) -> Vec<Expr> {
    let mut out = exprs.into_iter().fold(Vec::new(), |mut acc, expr| {
...
fn canonical_binding(binding: &Binding) -> Binding {
    Binding {
        exprs: normalize_exprs(binding.exprs.iter().map(canonical_expr)),
        space: binding.space.clone(),
    }
}

fn bindings_overlap(left: &Binding, right: &Binding) -> bool {
    left.space == right.space
        && left
            .exprs
            .iter()
            .any(|expr| right.exprs.contains(expr))
}

pub fn normalize_bindings(bindings: &[Binding]) -> Vec<Binding> {
    let mut acc = bindings
        .iter()
        .map(canonical_binding)
        .fold(Vec::new(), |mut acc, binding| {
    ...
    while changed {
        ...
        'outer: for i in 0..acc.len() {
            for j in (i + 1)..acc.len() {
                if bindings_overlap(&acc[i], &acc[j]) {
                    let mut exprs = acc[i].exprs.clone();
                    exprs.extend(acc[j].exprs.clone());
                    acc[i] = canonical_binding(&Binding {
                        exprs,
                        space: acc[i].space.clone(),
                    });
                    acc.remove(j);
                    changed = true;
                    break 'outer;
...
pub fn normalize_claim(claim: &OwnerClaim) -> OwnerClaim {
    normalize_exprs(claim.iter().map(canonical_expr))
}
...
fn canonical_binding_representative(binding: &Binding) -> Option<Expr> {
    canonical_binding(binding).exprs.into_iter().next()
}

pub fn normalize_claim_with_bindings(
    claim: &OwnerClaim,
    bindings: &[Binding],
    protect: &str,
) -> OwnerClaim {
    let protect_bindings = filter_bindings(&normalize_bindings(bindings), protect);
    ...
    let mut out = claim
        ...
        .map(|expr| {
            protect_bindings
                .iter()
                .find(|binding| binding.exprs.contains(&expr))
                .and_then(canonical_binding_representative)
                .unwrap_or(expr)
        })
...
pub fn normalize_claims_with_bindings(
    claims: &[OwnerClaim],
    bindings: &[Binding],
    protect: &str,
) -> Vec<OwnerClaim> {
    claims.iter().fold(Vec::new(), |mut acc, claim| {
        let claim = normalize_claim_with_bindings(claim, bindings, protect);
        if !acc.contains(&claim) {
            acc.push(claim);
        }
        acc
...
pub fn expr_equivalent_under_bindings(
    left: &Expr,
    right: &Expr,
    bindings: &[Binding],
    protect: &str,
) -> bool {
    normalize_claim_with_bindings(&vec![left.clone()], bindings, protect)
        == normalize_claim_with_bindings(&vec![right.clone()], bindings, protect)
}

pub fn transportable_owner_factor(expr: &Expr) -> Option<Expr> {
    match canonical_expr(expr) {
        Expr::Column(name) => Some(Expr::Column(name)),
        _ => None,
    }
}

/// OwnerClaim factors are row-charge facts. A transformation may preserve a
/// factor, substitute it with an equivalent expression through `Binding`, or
/// union it with another input claim at a join. A transformation must not
/// create a new owner factor solely because an expression identifies the
/// protected space.
///
/// OwnerClaim transport invariant:
///
/// Before truncation, each row must carry at least one valid `OwnerClaim`.
/// Transformations may construct output claims only by:
///   1. preserving existing owner factors,
///   2. rewriting existing factors through aliases / output expressions,
///   3. unioning claims at joins,
///   4. deduplicating factors through frame-local `Binding`s.
///
/// This is metadata-only. Do not materialize hidden owner columns just to
/// carry claims through the plan.
///
/// OwnerClaim factors may be arbitrary expressions at schema-compilation or
/// validation time, but any factor that must be transported through joins or
/// group-bys must resolve to a simple output column expression. Composite or
/// derived identifiers should be materialized or aliased before relational
/// query-plan transport.
pub fn rewrite_claim_to_keys(
    claim: &OwnerClaim,
    keys: &[Expr],
    bindings: &[Binding],
    protect: &str,
    ...
    let rewritten = claim
        ...
        .map(|factor| {
            keys.iter().find_map(|key| {
                expr_equivalent_under_bindings(factor, key, bindings, protect)
                .then(|| key.clone())
            })
        })
    ...
    let rewritten = rewritten
        ...
        .map(|expr| {
            transportable_owner_factor(&expr).ok_or_else(|| {
                err!(
                    MakeTransformation,
                    "owner claim factors transported through group_by must resolve to simple output columns; materialize or alias derived identifiers first"
                )
            })
        })
        .collect::<Fallible<Vec<_>>>()?;

    Ok(Some(normalize_claim_with_bindings(
        &rewritten,
        bindings,
        protect,
    )))
...
/// Convenience default for simple schemas: every protected binding is treated
/// as a singleton row owner.
///
/// This is not a general semantic rule. An expression may identify a protected
/// entity without making the row owned by that entity. Complex relational
/// schemas should provide explicit `Ownership` declarations.
...
pub fn compile_database_id_distance(
    ...
    for (table, bindings) in &compiled {
        ...
        let claims = normalize_claims_with_bindings(&claims, bindings, &protect);
...
pub trait PolarsMetric: MicrodataMetric {
    /// Frame-local alias/equality classes carried by this metric.
    fn bindings(&self) -> Vec<Binding>;
    /// Valid owner claims currently available on rows in this frame.
    ///
    /// Before truncation, each row should retain at least one valid owner
    /// claim. The empty default means this metric does not carry ownership
    /// information.
    fn owner_claims(&self) -> Vec<OwnerClaim> {
        vec![]
    }
    /// Name of the protected identifier space, when this metric tracks one.
    fn protected_label(&self) -> Option<&str> {
        None
    }
    /// Bindings in the active protected space.
    fn active_bindings(&self) -> Vec<Binding> {
        self.protected_label()
            .map(|label| filter_bindings(&self.bindings(), label))
            .unwrap_or_default()
    }
    /// Backward-compatible alias for protected-space bindings.
    fn active_id_sites(&self) -> Vec<Binding> {
        self.active_bindings()
    }
    /// Rebuild this metric across a grouping boundary.
    ///
    /// For identifier metrics, this is the owner-claim preservation step:
    /// grouping is allowed before truncation only when the grouped rows still
    /// carry a complete owner claim.
...
/// # Proof Definition
///
/// Under an identifier metric, each row must carry a valid `OwnerClaim`
/// until truncation. Operators before truncation are stable when they
/// preserve, refine, or soundly update this claim.
///
/// - selection and row-preserving projection preserve claims
/// - joins union claims and deduplicate factors through frame-local `Binding`s
/// - owner-preserving aggregation is allowed when the grouping keys preserve
///   a complete `OwnerClaim`
/// - truncation consumes the `OwnerClaim` and converts the data into bounded
///   event/group-level distance
///
/// ## `d`-closeness
...
impl PolarsMetric for SymmetricIdDistance {
    ...
    fn rebuild_group_by_metric(&self, keys: &[Expr]) -> Fallible<Self> {
        ...
        let surviving_bindings = normalize_bindings(&self
            .bindings
            .iter()
            .filter_map(|binding| {
                let exprs = binding
        ...
        let surviving_claims = self
            ...
            .iter()
            .filter_map(|claim| {
                rewrite_claim_to_keys(claim, &keys, &self.bindings, &self.protect).transpose()
            })
            .collect::<Fallible<Vec<_>>>()?;

        let surviving_claims =
            normalize_claims_with_bindings(&surviving_claims, &surviving_bindings, &self.protect);

        // Owner-preserving aggregation: group-by under an identifier metric is
        // valid when the grouping keys preserve a complete OwnerClaim. This
        // supports nested rollups such as events -> users, where grouping by
        // user_id preserves OwnerClaim([user_id]).
        //
        // Owner-erasing aggregation, such as grouping events by item while
        // dropping user_id, must instead be handled by an owner-aware DP
        // aggregation pipeline: partial aggregate by (item, owner claim),
        // truncate, then final aggregate.
