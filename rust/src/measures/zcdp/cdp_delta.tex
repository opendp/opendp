...
Finally, directed interval exponentiation rounds upwards, including when the
result is below the smallest positive \texttt{f64}; the result is clamped to
the probability range. Any interval-evaluation failure is returned as an
error rather than being used as a privacy result. Thus every successful
returned \texttt{delta} satisfies the postcondition.
\end{proof}
