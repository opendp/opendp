...
$1 - \delta - \exp(\epsilon)\alpha$ because \texttt{self.exp\_eps\_up} is an
upper bound and $\alpha$ is nonnegative. The second branch is similarly no
greater than its exact counterpart because \texttt{self.exp\_neg\_eps\_down}
is a lower bound. Taking the maximum with zero preserves the pointwise lower
bound, so the returned value is a conservative tradeoff guarantee.
\end{proof}
