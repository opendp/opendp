...
use crate::{
    ...
    measures::{
        curves::{
            PrivacyProfile, RenyiFn, check_alpha, check_epsilon, logspace::check_delta,
...
// with certified intervals where it matters.
...
pub fn beta_via_renyiDP(curve: Arc<RenyiFn>, source_delta: f64, alpha: f64) -> Fallible<f64> {
    ...
    if source_delta != 0.0 {
        let profile = PrivacyProfile::new({
            ...
            move |epsilon| delta_via_renyiDP(curve.as_ref(), source_delta, epsilon)
        });
        return beta_via_profile(&profile, alpha);
    }
