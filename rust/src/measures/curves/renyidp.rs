use crate::{
    error::Fallible,
    measures::{
        curves::{check_alpha, check_epsilon},
        zcdp_delta,
    },
    traits::DInterval,
...
pub fn delta_via_zCDP(rho: f64, epsilon: f64) -> Fallible<f64> {
    // Keep PrivacyCurve's zCDP representation identical to the canonical
    // certified conversion used by the zCDP measure cast.
    zcdp_delta(rho, epsilon)
}
