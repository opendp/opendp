...
pub fn get_posterior_curve(
    tradeoff_curve: Function<f64, f64>,
    prior: f64,
...
/// * `tradeoff_curve` - Tradeoff curve for the measurement
/// * `prior` - Attacker's prior probability.
pub fn get_relative_risk_curve(
    tradeoff_curve: Function<f64, f64>,
    prior: f64,
) -> Fallible<Function<f64, f64>> {
    Ok(Function::new_fallible(move |alpha: &f64| {
...
fn test_all() -> Fallible<()> {
    ...
    // Posterior
    let posterior_curve: Function<f64, f64> =
        get_posterior_curve(tradeoff_curve.clone(), 0.5).unwrap();
    println!(
        "posterior(prior=0.5, alpha=0) = {}",
        posterior_curve.eval(&0.0).unwrap()
    );
    println!(
        "posterior(prior=0.5, alpha=0.5) = {}",
        posterior_curve.eval(&0.5).unwrap()
    );
    println!(
        "posterior(prior=0.5, alpha=1) = {}",
        posterior_curve.eval(&0.5).unwrap()
    );

    // Relative risk

    let relative_risk_curve: Function<f64, f64> =
        get_relative_risk_curve(tradeoff_curve.clone(), 0.5).unwrap();
    println!(
        "relative_risk(prior=0.5, alpha=0) = {}",
        relative_risk_curve.eval(&0.0).unwrap()
    );
    println!(
        "relative_risk(prior=0.5, alpha=0.5) = {}",
        relative_risk_curve.eval(&0.5).unwrap()
    );
    println!(
