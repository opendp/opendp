...
use crate::{
    accuracy::{gaussian_scale_to_accuracy, laplacian_scale_to_accuracy},
    core::{Measure, Measurement, Metric, MetricSpace},
    domains::LazyFrameDomain,
    error::Fallible,
    measurements::{
        expr_noise::{match_noise, Distribution},
    ...
    if let Some((input, plugin)) = match_noise(&expr)? {
        ...
        let accuracy = if let Some(alpha) = alpha {
            Some(match distribution {
                Distribution::Laplace => laplacian_scale_to_accuracy(scale, alpha),
                Distribution::Gaussian => gaussian_scale_to_accuracy(scale, alpha),
            }?)
        } else {
            None
        };
