    ...
    Expr: StableExpr<PartitionDistance<MI>, PartitionDistance<MI>>,
{
    ...
    let Field { name, dtype } = &middle_domain.active_series()?.field;
    ...
    let stability_map = match dtype {
        ...
        Unknown(_) => return fallible!(MakeTransformation, "sum requires input data type to be statically known. Cast your data first: `.cast(dtype)`."),
...
fn sum_components<TI>(name: &SmartString) -> Fallible<(SeriesDomain, Expr)>
where
    TI: Summand,
    TI::Sum: Zero,
{
    Ok((
        SeriesDomain::new(name.as_str(), AtomDomain::<TI::Sum>::default()),
        lit(TI::Sum::zero()),
    ))
}
...
{
    let series_domain = domain.active_series()?;
    let margin = domain.active_margin()?;
