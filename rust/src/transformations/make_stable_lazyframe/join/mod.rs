...
use crate::{
    ...
    domains::{Database, DatabaseDomain, DslPlanDomain, FrameDomain, SeriesDomain},
    error::*,
    metrics::{unique_id_expr, DatabaseIdDistance, FrameDistance, IdSite, PolarsMetric, SymmetricIdDistance},
};

use super::StableDatabaseDslPlan;
...
fn output_domain_from_join(
    ...
    let series_domains: Vec<SeriesDomain> = schema
        .iter_fields()
...
fn merge_private_id_sites(
    ...
    let protected_label = left_metric.0.protected_label.clone();
    let left_active = left_metric.0.active_id_sites();
    let right_active = right_metric.0.active_id_sites();
    let left_expr = unique_id_expr(&left_active)?
        .ok_or_else(|| err!(MakeTransformation, "left private branch is missing a protected identifier site"))?;
    let right_expr = unique_id_expr(&right_active)?
        .ok_or_else(|| err!(MakeTransformation, "right private branch is missing a protected identifier site"))?;
    ...
    let mut output_sites = vec![IdSite {
    ...
    output_sites.extend(
        left_metric
            ...
            .id_sites
            .iter()
            .chain(right_metric.0.id_sites.iter())
            .filter(|site| site.label != protected_label)
            .cloned(),
    );
...
> {
    ...
    let t_left = input_left
        .as_ref()
        .clone()
        .make_stable_database(input_domain.clone(), input_metric.clone())?;
    ...
    let (output_metric, map_left) = match (left_private, right_private) {
        (true, false) => (
            FrameDistance(SymmetricIdDistance {
                protected_label: left_metric.0.protected_label.clone(),
                id_sites: augmented_id_sites(left_metric.0.id_sites.clone(), &left_on, &right_on),
            }),
            true,
        ),
        (false, true) => (
            FrameDistance(SymmetricIdDistance {
                protected_label: right_metric.0.protected_label.clone(),
                id_sites: augmented_id_sites(right_metric.0.id_sites.clone(), &right_on, &left_on),
            }),
            false,
        ),
        (true, true) => {
            let left_id = unique_id_expr(&left_metric.0.active_id_sites())?
                .ok_or_else(|| err!(MakeTransformation, "left private branch is missing a protected identifier site"))?;
            let right_id = unique_id_expr(&right_metric.0.active_id_sites())?
                .ok_or_else(|| err!(MakeTransformation, "right private branch is missing a protected identifier site"))?;
            ...
            (
                FrameDistance(SymmetricIdDistance {
                    protected_label: left_metric.0.protected_label.clone(),
                    id_sites: merge_private_id_sites(&left_metric, &right_metric)?,
                }),
                true,
            )
        }
    ...
    let f_left = t_left.function_clone();
    let f_right = t_right.function_clone();
    let m_left = t_left.stability_map_clone();
    let m_right = t_right.stability_map_clone();
