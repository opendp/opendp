...
use crate::{
    ...
    metrics::{Binding, Bound, Bounds, DatabaseIdDistance, PolarsMetric},
};
use polars::prelude::*;
...
#[test]
fn test_database_source_preserves_non_protected_sites_but_only_activates_protected_label()
-> Fallible<()> {
    let events_domain = LazyFrameDomain::new(vec![
        SeriesDomain::new("user_id", AtomDomain::<i32>::default()),
    ...
    assert_eq!(t_source.output_metric.0.bindings.len(), 2);
...
fn test_database_source_requires_marker_for_multitable_scan() -> Fallible<()> {
    ...
    let database_metric = DatabaseIdDistance {
        ...
        base_owner_claims: HashMap::new(),
    };

    let unmarked_scan = df!("user_id" => [1i32], "value" => [10i32])?
        .lazy()
        .logical_plan;
    let err =
        make_stable_database_source(database_domain, database_metric, unmarked_scan).unwrap_err();
    assert!(
        err.message
            .unwrap_or_default()
