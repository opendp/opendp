...
use crate::{
    core::Transformation,
    domains::{AtomDomain, DatabaseDomain, DslPlanDomain, LazyFrameDomain, Margin, OptionDomain, SeriesDomain},
    metrics::{Bounds, DatabaseIdDistance, FrameDistance, IdSite, SymmetricDistance, SymmetricIdDistance},
    transformations::{StableDatabaseDslPlan, StableDslPlan, make_stable_lazyframe},
};
...
fn test_database_truncation_matches_single_table_stability_map() -> Fallible<()> {
    ...
    let t_database: Transformation<
        ...
        DslPlanDomain,
        FrameDistance<SymmetricDistance>,
    > = database_plan.make_stable_database(database_domain, database_metric)?;
...
fn test_database_truncation_rejects_multiple_protected_identifier_exprs() -> Fallible<()> {
    ...
    let err = <DslPlan as StableDatabaseDslPlan<
        DatabaseIdDistance,
        FrameDistance<SymmetricDistance>,
    >>::make_stable_database(plan, database_domain, database_metric)
    .unwrap_err();
