...
use crate::error::*;
use crate::metrics::{
    Bound, Bounds, FrameDistance, L0PInfDistance, L01InfDistance, PolarsMetric, SymmetricDistance,
    SymmetricIdDistance, unique_id_expr,
};
...
{
    let identifier =
        unique_id_expr(&t_prior.output_metric.0.active_id_sites())?.ok_or_else(|| {
            err!(
                MakeTransformation,
