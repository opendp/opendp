...
pub(crate) fn find_truncation_claim(
    ...
    // OwnerClaim supports multi-owner metadata and propagation. The current
    // truncation implementation supports only singleton owner claims.
    // Multi-owner claims are preserved for future support, but cannot yet be
    // consumed by this truncator.
