...
use crate::{
    ...
    metrics::{
        ...
        HammingDistance, PolarsMetric, SymmetricDistance, SymmetricIdDistance, filter_id_sites,
        unique_id_expr,
    },
...
pub(crate) fn database_metric(input_metric: &DatabaseIdDistance) -> SymmetricIdDistance {
    SymmetricIdDistance {
        ...
        bindings: input_metric
            .bindings
            .values()
            .flat_map(|sites| filter_id_sites(sites, &input_metric.protect))
            .collect(),
    }
}
    ...
    > for DslPlan
{
    ...
    > {
        ...
        let identifier = unique_id_expr(&input_metric.0.active_id_sites())?.ok_or_else(|| {
            err!(
                MakeTransformation,
                "truncation requires at least one identifier"
            )
        })?;
    ...
    for DslPlan
{
    ...
    > {
        ...
        if let Some(identifier) =
            crate::metrics::unique_id_expr(&database_metric(&input_metric).active_id_sites())?
        {
