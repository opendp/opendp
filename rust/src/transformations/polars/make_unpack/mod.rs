...
use crate::{
    ...
    domains::{AtomDomain, OptionDomain, PrimitiveDataType, SeriesDomain, VectorDomain},
    error::Fallible,
    traits::CheckAtom,
};
...
impl ToVec<String> for ChunkedArray<StringType> {
    ...
    (VectorDomain<OptionDomain<AtomDomain<T>>>, M): MetricSpace,
{
    ...
    Transformation::new(
        input_domain.clone(),
        output_domain,
        Function::new_fallible(move |arg: &Series| arg.unpack::<T::Polars>()?.to_option_vec()),
        input_metric.clone(),
        input_metric,
    ...
    (VectorDomain<AtomDomain<T>>, M): MetricSpace,
{
    ...
    if input_domain.nullable {
        return fallible!(
            MakeTransformation,
            "data must be non-null. Use make_series_to_option_vec."
        );
    }
    ...
    Transformation::new(
        input_domain,
        output_domain,
        Function::new_fallible(move |arg: &Series| arg.unpack::<T::Polars>()?.to_vec()),
        input_metric.clone(),
        input_metric,
        StabilityMap::new(M::Distance::clone),
    )
