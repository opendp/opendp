...
use crate::domains::{Bounds, DynSeriesAtomDomain, ExprDomain, NumericDataType, OuterMetric};
use crate::error::*;
    ...
    (ExprDomain<M::LazyDomain>, M): MetricSpace,
{
    ...
    Transformation::new(
        ...
        Function::new_expr(move |expr| {
            expr.clip(lit(bounds.0), lit(bounds.1))
        }),
        input_metric.clone(),
        input_metric,
...
mod test_make_clamp {
    ...
    fn test_make_clamp_expr() -> Fallible<()> {
        ...
        let expr_exp = col(active_col).clip(lit(1), lit(3));
