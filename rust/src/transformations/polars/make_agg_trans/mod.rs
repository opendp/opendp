...
use crate::domains::{
    ExprDomain, ExprMetric, LazyFrameDomain, LazyGroupByContext, LazyGroupByDomain,
};
use crate::error::*;
...
pub fn make_agg_trans<T: AggTransformation>(
    ...
    Transformation<
        ...
        LazyFrameDomain,
        T::InputMetric,
        <T::OutputMetric as ExprMetric<LazyGroupByContext>>::InnerMetric,
    >,
>
where
...
mod test_make_agg_trans {
    ...
    fn test_make_agg_trans_output_lazy_frame() -> Fallible<()> {
        let (expr_domain_base, lazy_frame) = get_test_data()?;
