...
use crate::{
    ...
    domains::{AtomDomain, ExprDomain, OuterMetric, PrimitiveDataType, VectorDomain},
    error::Fallible,
    measurements::DExpOuterMetric,
    metrics::{InsertDeleteDistance, IntDistance, LInfDistance, SymmetricDistance, L1},
    traits::{CheckAtom, ExactIntCast, InfCast, Number},
    transformations::{make_quantile_score_candidates, traits::UnboundedMetric, ToVec},
};
...
pub fn make_quantile_score_candidates_expr<MI, TIA: CheckAtom + Number + PrimitiveDataType>(
...
pub trait DQuantileOuterMetric: OuterMetric<Distance = IntDistance> {
    type ScoreMetric: DExpOuterMetric<
        // the inner metric will always be the L_{\infty} distance
        InnerMetric = LInfDistance<Score>,
...
impl DQuantileOuterMetric for InsertDeleteDistance {
    type ScoreMetric = LInfDistance<Score>;
}
impl DQuantileOuterMetric for L1<InsertDeleteDistance> {
    type ScoreMetric = L1<LInfDistance<Score>>;
}
