...
use crate::traits::{CheckAtom, InfAdd, InfCast, InfMul, InfSub, Number, TotalOrd, ExactIntCast};
    ...
    (ExprDomain<MI::LazyDomain>, MI::SumMetric): MetricSpace,
{
    ...
    let max_partitions = item::<u32>(input_margin.data.clone().select([count()]))?;
...
mod test_make_sum_expr {
    ...
    fn test_make_sum_expr_output_domain() -> Fallible<()> {
        ...
        let margin =
            Margin::new_from_counts(df!("A" => &[1, 2], "count" => &[1u32, 1])?.lazy(), 1)?;
