use crate::{
    samplers::{SampleBernoulli, SampleStandardBernoulli, SampleUniformInt},
    traits::FloatBits,
};
...
use super::SampleUniformExponent;
...
fn sample_geometric_linear_time<T, S>(
    mut shift: T,
    positive: bool,
    prob: S,
    mut trials: Option<T>,
...
where
    T: Clone + Zero + One + PartialEq + AddAssign + AddAssign + Bounded + AddAssign + SubAssign,
    S: num::Float + Copy + One + Zero + PartialOrd + SampleUniformExponent,
    S::Bits: PartialOrd,
{
...
// pub fn sample_geometric_log_time<T, S: FloatBits>(shift: T, positive: bool, prob: S) -> Fallible<T>
// where
