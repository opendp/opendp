...
#[bootstrap(
    ...
    arguments(
        ...
        privacy_measure(hint = "Measure"),
        function(rust_type = "$pass_through(TO)"),
        privacy_map(rust_type = "$measure_distance_type(privacy_measure)"),
    ),
...
/// * `privacy_measure` - The measure from which distances between adjacent output distributions are measured.
/// * `function` - A function mapping data from `input_domain` to a release of type `TO`.
/// * `privacy_map` - A function mapping distances from `input_metric` to `privacy_measure`.
///
/// # Generics
...
pub extern "C" fn opendp_internal___make_measurement(
    ...
    Measurement::new(
        ...
        try_as_ref!(privacy_measure).clone(),
