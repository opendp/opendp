...
use crate::{
    ...
    utilities::{
        BinarySearchable, fallible_binary_search, fallible_binary_search_by_with_range_errors,
...
pub extern "C" fn opendp_internal___binary_search_by(
        ...
        T: BinarySearchable + 'static + Send + Sync,
    {
        ...
        let lower = as_ref(lower as *const T).cloned();
        let upper = as_ref(upper as *const T).cloned();
        fallible_binary_search_by_with_range_errors(comparison, (lower, upper)).map(AnyObject::new)
    }
