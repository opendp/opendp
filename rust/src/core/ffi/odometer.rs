...
use crate::{
    combinators::{
        ffi::{AnyOdometerAnswer, AnyOdometerQuery},
        Invokable,
    },
    core::{Domain, Function, Measure, Metric, MetricSpace, Odometer},
    error::Fallible,
    ffi::{
        any::{
            AnyDomain, AnyMeasure, AnyMeasurement, AnyMetric, AnyObject, AnyOdometer, AnyQueryable,
            Downcast,
        },
        util,
    },
...
#[bootstrap(
    name = "odometer_queryable_invoke_measurement",
    arguments(queryable(rust_type = b"null"))
)]
/// Eval the odometer `queryable` with an invoke `query` that is a measurement. Returns a differentially private release.
///
/// # Arguments
/// * `queryable` - Queryable to eval.
/// * `query` - Invoke query to supply to the queryable.
#[no_mangle]
pub extern "C" fn opendp_core__odometer_queryable_invoke_measurement(
    queryable: *mut AnyObject,
    measurement: *const AnyMeasurement,
) -> FfiResult<*mut AnyObject> {
    let queryable = try_!(try_as_mut_ref!(queryable).downcast_mut::<AnyQueryable>());

    let measurement = try_as_ref!(measurement).clone();
    let invokable = Invokable::Measurement(measurement);

    // this.invoke(&AnyObject::new(invokable)).into()
    let query = AnyObject::new(AnyOdometerQuery::Invoke(AnyObject::new(invokable)));
...
#[bootstrap(
    name = "odometer_queryable_invoke_odometer",
    arguments(queryable(rust_type = b"null"))
)]
...
/// * `query` - Invoke query to supply to the queryable.
#[no_mangle]
pub extern "C" fn opendp_core__odometer_queryable_invoke_odometer(
    queryable: *mut AnyObject,
    ...
    let invokable = Invokable::Odometer(odometer);
...
mod tests {
    ...
    fn make_test_any_odometer() -> Fallible<AnyOdometer> {
        Odometer::new(
            ...
            Function::new_wrappable(|arg: &AnyObject, wrapper: Option<WrapFn>| {
                let data = arg.downcast_ref::<Vec<i32>>()?.clone();
                Queryable::new_external(
                    move |query: &OdometerQuery<AnyObject, AnyObject>, _| {
                        Ok(match query {
