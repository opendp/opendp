...
use crate::error::*;
use crate::interactive::{Queryable, WrapFn};
...
pub struct Function<TI, TO> {
    pub function: Arc<dyn Fn(&TI, Option<WrapFn>) -> Fallible<TO> + Send + Sync>,
}
...
impl<TI, TO> Function<TI, TO> {
    ...
    pub fn new_fallible(function: impl Fn(&TI) -> Fallible<TO> + 'static + Send + Sync) -> Self {
        Self::new_wrappable(move |arg, _| function(arg))
    }

    pub fn new_wrappable(
        function: impl Fn(&TI, Option<WrapFn>) -> Fallible<TO> + 'static + Send + Sync,
    ) -> Self {
        Self {
    ...
    pub(crate) fn eval_wrap(&self, arg: &TI, wrapper: Option<WrapFn>) -> Fallible<TO> {
...
impl<TI: 'static, TO: 'static> Function<TI, TO> {
    pub fn make_chain<TX: 'static>(
        ...
        Self::new_wrappable(move |arg, wrapper| {
...
impl<DI: Domain, TO, MI: Metric, MO: Measure> Measurement<DI, TO, MI, MO> {
    ...
    pub(crate) fn invoke_wrap(&self, arg: &DI::Carrier, wrapper: Option<WrapFn>) -> Fallible<TO> {
