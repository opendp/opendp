...
use crate::domains::{DatasetMetric, Frame, FrameDomain};
use crate::{
...
impl<D: DatasetMetric, F: Frame> MetricSpace for (ParquetDomain<F>, D) where (FrameDomain<F>, D): MetricSpace{
    fn check(&self) -> bool {
        (self.0.frame_domain.clone(), self.1.clone()).check()
    }
}
