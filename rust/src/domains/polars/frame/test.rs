...
fn test_database_metric_space_validation() -> Fallible<()> {
    ...
    assert!(
        (database_domain.clone(), valid_metric)
            .check_space()
            .is_ok()
    );
    ...
    assert!(
        (database_domain.clone(), invalid_metric)
            .check_space()
            .is_err()
    );
    ...
    assert!(
        (database_domain, missing_table_metric)
            .check_space()
            .is_err()
    );
