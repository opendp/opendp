...
/// # Proof Definition
/// Return a subset of `sets` whose intersection is a superset of `elements`.
///
        ...
        let (best_set, weight) = sets
            ...
            .max_by_key(|(set, len)| {
                (
                    ...
                    set.intersection(&not_covered).count(),
            ...
            .and_then(|(&best_set, weight)| {
                (!best_set.is_disjoint(&not_covered)).then(|| (best_set, *weight))
            })?;

        not_covered.retain(|x| !best_set.contains(x));
