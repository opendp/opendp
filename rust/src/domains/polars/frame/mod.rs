...
impl MetricSpace for (DatabaseDomain, DatabaseIdDistance) {
    fn check_space(&self) -> Fallible<()> {
        ...
        for (table_name, id_sites) in &self.1.id_sites {
            ...
            has_protected_site |= id_sites
                .iter()
                .any(|site| site.label == self.1.protected_label);
...
impl<M: MicrodataMetric> MetricSpace for (DatabaseDomain, M) {
    fn check_space(&self) -> Fallible<()> {
        self.0
            .0
            .values()
            .try_for_each(|domain| (domain.clone(), self.1.clone()).check_space())
    }
}
...
impl<M: UnboundedMetric> MetricSpace for (DatabaseDomain, FrameDistance<M>) {
    fn check_space(&self) -> Fallible<()> {
        self.0
            .0
            .values()
...
impl DatabaseDomain {
    ...
    pub fn get(&self, table_name: &str) -> Fallible<LazyFrameDomain> {
        self.0
            .get(table_name)
            .cloned()
            .ok_or_else(|| err!(MakeTransformation, "unrecognized table domain: {}", table_name))
    }
}
...
pub(crate) fn table_name_from_schema_or_domain(
    ...
    table_name_from_schema(schema).or_else(|_| match database_domain.0.len() {
        1 => database_domain
            ...
            .next()
            .cloned()
            .ok_or_else(|| err!(MakeTransformation, "database domain unexpectedly had no tables")),
        _ => fallible!(
            MakeTransformation,
...
pub(crate) fn strip_table_markers(df: &DataFrame) -> Fallible<DataFrame> {
    table_marker_columns(df.schema())
        .into_iter()
        .try_fold(df.clone(), |acc, marker| acc.drop(marker.as_str()).map_err(Into::into))
}
...
impl Margin {
    ...
    pub(crate) fn member(&self, value: LazyGroupBy) -> Fallible<bool> {
        ...
        let max_part_length = max_part_length.get_columns()[0]
            .u32()?
            .get(0)
            .unwrap_or(0);
        ...
        let max_num_parts = max_num_parts.get_columns()[0]
            .u32()?
            .get(0)
            .unwrap_or(0);
