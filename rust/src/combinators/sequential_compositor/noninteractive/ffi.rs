...
pub extern "C" fn opendp_combinators__make_basic_composition(
    ...
    let measurements: Vec<AnyMeasurement> =
        try_!(meas_ptrs.iter().map(|ptr| Ok(try_as_ref!(*ptr).clone())).collect());
    ...
    fn concurrent(&self) -> bool {
        fn monomorphize1<Q: 'static + Clone + InfAdd + Zero>(
            ...
            where
                M::Distance: Clone,
            {
                Ok(self_.downcast_ref::<M>()?.concurrent())
            }
            dispatch!(monomorphize2, [
                (self_.type_, [MaxDivergence<Q>, FixedSmoothedMaxDivergence<Q>, ZeroConcentratedDivergence<Q>])
            ], (self_))
        }
        let Ok(Q_Atom) = self.type_.get_atom() else {
            return false;
        };
        dispatch!(monomorphize1, [(Q_Atom, @floats)], (self)).unwrap_or(false)
    }
