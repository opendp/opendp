...
use crate::{
    ...
    error::Fallible,
    interactive::{merge_wrappers, Answer, Query, Queryable, WrapFn},
    traits::ProductOrd,
};
    ...
    (DI, MI): MetricSpace,
{
    ...
    Measurement::new(
        ...
        Function::new_wrappable(enclose!(
            ...
            move |arg: &DI::Carrier, wrapper_outer: Option<WrapFn>| {
                ...
                Queryable::new(
                    move |sc_qbl, query: Query<Measurement<DI, TO, MI, MO>>| {
                        ...
                        if let Query::External(measurement, wrapper_inner) = query {
                            ...
                            let wrapper = (!output_measure.concurrent()?).then(|| {
                                ...
                                WrapFn::new_pre_hook(move || {
                                    sc_qbl.eval_internal(&AskPermission(child_id))
                                })
                            });

                            let answer = measurement
                                .invoke_wrap(&arg, merge_wrappers(wrapper, wrapper_inner))?;
