...
fn make_approxDP_to_profileDP(measurement: &AnyMeasurement) -> Fallible<AnyMeasurement> {
    ...
    let measurement = super::make_approxDP_to_profileDP(measurement)?;
    let privacy_map = measurement.privacy_map.clone();
...
#[bootstrap(name = "make_approxDP_to_profileDP", features("contrib"))]
/// Constructs a new output measurement where the output measure
/// is cast from `ApproxDP` to `ProfileDP`.
///
/// # Arguments
/// * `measurement` - a measurement with a privacy measure to be cast
#[unsafe(no_mangle)]
#[allow(non_snake_case)]
pub extern "C" fn opendp_combinators__make_approxDP_to_profileDP(
    measurement: *const AnyMeasurement,
) -> FfiResult<*mut AnyMeasurement> {
    make_approxDP_to_profileDP(try_as_ref!(measurement)).into()
}
...
pub extern "C" fn opendp_combinators__make_fixed_approxDP_to_approxDP(
    ...
    opendp_combinators__make_approxDP_to_profileDP(measurement)
}
