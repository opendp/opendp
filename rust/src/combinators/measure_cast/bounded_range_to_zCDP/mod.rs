use crate::{
    ...
    error::Fallible,
    measures::{ZeroConcentratedDivergence, BoundedRange},
    traits::Float,
};
    ...
    (DI, MI): MetricSpace,
{
    ...
    let constant: QO = QO::one() + QO::one() + QO::one() + QO::one() + QO::one() + QO::one() + QO::one() + QO::one();

    m.with_map(
        m.input_metric.clone(),
        ZeroConcentratedDivergence::default(),
        PrivacyMap::new_fallible(move |d_in: &MI::Distance| {
            privacy_map.eval(d_in).map(|br_eps| br_eps.powi(2) / constant)
        }),
    )
}
