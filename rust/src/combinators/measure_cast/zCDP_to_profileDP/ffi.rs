...
fn make_zCDP_to_profileDP(measurement: &AnyMeasurement) -> Fallible<AnyMeasurement> {
    fn monomorphize<MO: 'static + ConcentratedMeasure>(
        ...
        let meas = super::make_zCDP_to_profileDP(meas)?;
        let privacy_map = meas.privacy_map.clone();
        Measurement::new(
            meas.input_domain.clone(),
            meas.input_metric.clone(),
...
#[bootstrap(name = "make_zCDP_to_profileDP", features("contrib"))]
/// Constructs a new output measurement where the output measure
/// is cast from `zCDP` to `ProfileDP`.
///
/// # Arguments
/// * `measurement` - a measurement with a privacy measure to be cast
#[unsafe(no_mangle)]
#[allow(non_snake_case)]
pub extern "C" fn opendp_combinators__make_zCDP_to_profileDP(
    measurement: *const AnyMeasurement,
) -> FfiResult<*mut AnyMeasurement> {
    FfiResult::from(make_zCDP_to_profileDP(try_as_ref!(measurement)))
}
...
pub extern "C" fn opendp_combinators__make_zCDP_to_approxDP(
    ...
    opendp_combinators__make_zCDP_to_profileDP(measurement)
}
