use crate::{core::Function, error::Fallible, traits::Float};

pub(crate) fn rdp_epsilon<Q: Float>(curve: Function<Q, Q>, delta: Q) -> Fallible<Q> {
    ...
    let deriv_pos = |a: Q| Fallible::Ok(curve.eval(&a)? > -(a * delta).ln() / (a - _1).powi(2));
    //                                              ε   > -ln(αδ)           / (α - 1)^2
    ...
    //  epsilon = δ \bar{ε} +   (ln(1/δ) + (α-1) ln((α-1)/α) - ln(α)) / (α - 1)
    //                            ----------------------------------- /  -----
    //          = δ \bar{ε}                           +         numer / denom
    let epsilon = a_max
        .inf_mul(&curve.eval(&a_max)?)?
        .inf_add(&numer.inf_div(&denom)?)?;
...
mod test {
    ...
    fn test_edge_cases() -> Fallible<()> {
        // assert!(rdp_epsilon(-0., 0.).is_err());
        // assert!(rdp_epsilon(0., -0.).is_err());
        // assert_eq!(rdp_epsilon(0., 0.)?, 0.);
        // assert_eq!(rdp_epsilon(0.1, 0.)?, f64::INFINITY);
        // assert!(rdp_epsilon(0.1, 0.1)? > 0.);
        // assert_eq!(rdp_epsilon(0.1, 1.)?, 0.);
        // assert!(rdp_epsilon(0.1, 1.01).is_err());
        // assert_eq!(rdp_epsilon(f64::INFINITY, 1.)?, f64::INFINITY);
