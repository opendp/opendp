...
use crate::core::AnyOdometerQuery;
use crate::core::{FfiResult, Function, Odometer};

use crate::error::Fallible;
use crate::ffi::any::{AnyFunction, AnyMeasurement, AnyObject, AnyOdometer, AnyTransformation};
...
fn make_chain_ot(
    ...
    let odometer1 = Odometer::new(
        ...
        Function::new_interactive(move |arg: &AnyObject, wrapper| {
            let mut inner_qbl = odometer1_function.eval(arg)?;
            Queryable::new(move |_qbl, query: Query<AnyOdometerQuery>| {
                Ok(match query {
                    Query::External(external, inner_wrapper) => {
                        Answer::External(inner_qbl.eval_wrap(external, inner_wrapper)?)
                    }
    ...
    super::make_chain_ot(&odometer1, transformation0)
}
...
fn make_chain_po(function1: &AnyFunction, odometer0: &AnyOdometer) -> Fallible<AnyOdometer> {
    ...
    super::make_chain_po(&function1, &odometer0)
}
