...
pub trait Invokable<DI: Domain, DQ: Domain, DA: Domain, MI: Metric, MO: Measure> {
    fn invoke(&self, value: &DI::Carrier, context: Context) -> Fallible<Queryable<DQ::Carrier, DA::Carrier>>;
...
{
    fn invoke(&self, value: &DI::Carrier, _context: Context) -> Fallible<Queryable<DQ::Carrier, DA::Carrier>> {
        self.invoke(value)
    }
...
where
    ...
    DQ::Carrier: Clone
{
    fn invoke(&self, value: &DI::Carrier, mut context: Context) -> Fallible<Queryable<DQ::Carrier, DA::Carrier>> {
        let mut inner = self.invoke(value)?;

        Ok(Queryable::new(move |_self: &QueryableBase, query: &dyn Any| {
...
>(
    ...
    output_measure: MO,
    d_in: MI::Distance,
) -> Fallible<Odometer<DI, DQ, DA, MI, MO>>
where
            ...
            move |arg: &DI::Carrier| {
                ...
                Queryable::new(move |self_: &QueryableBase, query: &dyn Any| {
                    ...
                    if let Some(invokable) = query.downcast_ref::<DQ::Carrier>() {
                        ...
                        let answer = invokable.invoke(&arg, Context::new(self_.clone(), d_children.len()))?;
...
mod test {
    ...
    //     let mut answer4: Queryable<Measurement1<_, PolyDomain, _, _>, Box<dyn Any>> =
