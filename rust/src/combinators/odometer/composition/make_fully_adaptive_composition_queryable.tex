...
\begin{itemize}
    ...
    \item Argument \texttt{data} of type \texttt{DI\_Carrier}.
...
\lstinputlisting[language=Python,firstline=2,escapechar=`]{./pseudocode/make_fully_adaptive_composition_queryable.py}

\subsubsection*{Postconditions}
\validMeasurement{\texttt{(input\_domain, input\_metric, output\_measure, DI, TO, MI, MO)}}{\texttt{make\_fully\_adaptive\_composition\_queryable}}

\begin{proof} \textup{Wrapping Guarantee}
    The query wrapper is labeled \texttt{query\_wrapper} on line \ref{query-wrapper-def}.
    \texttt{wrapper} on line \ref{compose-wrappers} includes \texttt{query\_wrapper} 
    by the postcondition on \rustdoc{interactive/fn}{compose\_wrappers}.

    \rustdoc{core/struct}{Measurement}\texttt{.invoke\_wrap} on line \ref{invoke} guarantees to wrap all spawned IM queryables with the provided wrapper,
...
\begin{proof} \textup{Privacy guarantee}
    ...
    \begin{itemize}
        ...
        \item Instead of storing the privacy losses $d_k$ for some \din in the state,
        a vector of functions is stored that can compute $d_k$ when given \din.
        The choice of \din is left implicit in the paper, 
        in regards to $\mathcal{M}_k$ satisfying $d-\mathcal{D}$-DP;
        whereas \din is explicit and delayed in the implementation.
        The equivalent odometer state is now $(s, [\mathrm{map}_1(\cdot), ... \mathrm{map}_k(\cdot)])$.
    ...
    as well as a mutable list of privacy loss maps from line \ref{mutable-state},
    to form the state.
