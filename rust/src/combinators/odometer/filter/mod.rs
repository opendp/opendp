use crate::{
    ...
    error::Fallible,
    interactive::{Answer, IntoPolyQueryable, Query, Queryable, WrapFn},
    traits::ProductOrd,
};

use super::{ChildChange, IsInvokable, OdometerQueryable};

pub fn make_odometer_to_filter<
    DI: 'static + Domain,
    Q: 'static + IsInvokable<DI, MI, MO> + Clone,
...
where
    MI::Distance: 'static + Clone + Send + Sync + ProductOrd,
    DI::Carrier: Clone + Send + Sync,
    MO::Distance: Clone + ProductOrd,
    (DI, MI): MetricSpace,
{
    let Odometer {
        input_domain,
    ...
    Measurement::new(
        ...
        Function::new_wrappable(enclose!(
            ...
            move |arg: &DI::Carrier, wrapper: Option<WrapFn>| {
                ...
                let filter_queryable = Queryable::<(), ()>::new(
                    move |_self: &Queryable<_, _>, query: Query<()>| {
                ...
                function.eval_wrap(arg, Some(wrap_logic))
            }
        )),
        input_metric,
        output_measure,
