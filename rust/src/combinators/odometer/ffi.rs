...
use super::{Invokable, IsInvokable, OdometerAnswer, OdometerQuery};

pub type AnyOdometerQuery = OdometerQuery<AnyObject, AnyObject>;
pub type AnyOdometerAnswer = OdometerAnswer<AnyObject, AnyObject>;

impl IsInvokable<AnyDomain, AnyMetric, AnyMeasure> for AnyOdometer {
    ...
    fn invoke_wrap_and_map(
        ...
        let map = PrivacyMap::new_fallible(move |d_in: &AnyObject| {
            ...
            let AnyOdometerAnswer::Map(d_out) = inner_qbl
                .clone()
                .eval(&query)?
                .downcast::<AnyOdometerAnswer>()?
            else {
                return fallible!(FailedFunction, "expected odometer answer to be a map");
            };
            Ok(d_out)
        });
        Ok((answer, map))
...
pub type AnyInvokable = Invokable<AnyDomain, AnyObject, AnyMetric, AnyMeasure>;
