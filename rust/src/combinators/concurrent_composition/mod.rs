...
use crate::{
    ...
    error::Fallible,
    interactive::{Queryable, Query, Answer, IntoDyn, FromDyn},
    measures::{MaxDivergence, SmoothedMaxDivergence},
    traits::{TotalOrd, CheckNull},
};
...
where
    ...
    DQ::Carrier: CheckNull,
    Queryable<Measurement<DI, DQ, DA, MI, MO>, QueryableDomain<DQ, DA>>: IntoDyn + FromDyn,
{
    if d_mids.len() == 0 {
    ...
    Ok(Measurement::new(
        input_domain.clone(),
        AllDomain::<Measurement<_, _, _, _, _>>::new(),
        QueryableDomain::new(query_domain, answer_domain),
        Function::new(|arg| unimplemented!()),
...
//         let rr_dyn_query = make_randomized_response_bool(0.5, false)?.into_dyn();
//         let rr_query = make_randomized_response_bool(0.5, false)?;

//         // pass queries into the CC queryable
//         let _answer1: bool = queryable.eval(&rr_dyn_query)?.get_dyn()?;
//         let _answer2: bool = queryable.eval(&rr_dyn_query)?.get_dyn()?;
...
//         let mut answer3 = queryable.eval_dyn::<_, QueryableDomain<AllDomain<()>, AllDomain<bool>>>(&cc_query_3)?;
...
//         let mut answer4 = queryable.eval_dyn::<_, QueryableDomain<DynDomain, PolyDomain>>(&cc_query_4)?;
//         let _answer4_1: bool = answer4.eval(&rr_dyn_query)?.get_dyn()?;
//         let _answer4_2: bool = answer4.eval(&rr_dyn_query)?.get_dyn()?;
