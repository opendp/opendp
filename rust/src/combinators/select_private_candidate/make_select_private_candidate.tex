...
\begin{itemize}
    ...
    \item Argument \texttt{measurement} is a measurement whose output metric is \rustdoc{measures/struct}{PureDP}, and releases a tuple \texttt{(f64, TO)}, where \texttt{TO} is some arbitrary type.
...
if $x, x'$ are $\din$-close then $\function(x), \function(x')$ are $\texttt{privacy\_map}(\din)$-close under $\texttt{output\_measure}$ (pure-DP).
