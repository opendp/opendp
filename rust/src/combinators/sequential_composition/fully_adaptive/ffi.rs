...
#[unsafe(no_mangle)]
pub extern "C" fn opendp_combinators__make_fully_adaptive_composition_k(
    input_domain: *const AnyDomain,
    input_metric: *const AnyMetric,
    output_measure: *const AnyMeasure,
) -> FfiResult<*mut AnyOdometer> {
    ...
    let input_metric = try_as_ref!(input_metric).clone();
    let output_measure = try_as_ref!(output_measure).clone();
    super::make_fully_adaptive_composition_k(input_domain, input_metric, output_measure).into()
}
