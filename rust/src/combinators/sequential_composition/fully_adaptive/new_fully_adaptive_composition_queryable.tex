...
\begin{proof}[Proof of privacy guarantee]
    ...
    \begin{itemize}
        ...
        \item When the privacy loss is requested, each distinct privacy map is evaluated once,
        and the resulting distance is reused for repeated maps (on line \ref{distinct-maps}).
        Since privacy maps are deterministic functions,
        the \texttt{d\_mids} passed to \texttt{compose} are the same as if each map were evaluated individually.
