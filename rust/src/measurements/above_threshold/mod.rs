...
use crate::{
    ...
    traits::{samplers::PartialSample, InfCast, InfDiv, InfMul, Number},
};

use super::{Optimize, SelectionMeasure, ShiftScaleRV};

#[cfg(test)]
mod test;

pub fn make_above_threshold<MO: SelectionMeasure, SI, SO: Number>(
    ...
    f64: InfCast<SO>,
{
    ...
    Measurement::new(
        input_domain,
        Function::new_fallible(move |arg: &Queryable<SI, SO>| {
            ...
            Queryable::new_external(move |input: &SI| {
                ...
                let mut output = FBig::try_from(stream.eval(input)?).unwrap_or(FBig::ZERO);

                if let Optimize::Min = optimize {
                    output = -output;
                }

                let output_rv = MO::RV::new(output, f_scale.clone())?;
                let mut noisy_output = PartialSample::new(output_rv);

                if noisy_output.greater_than(&mut noisy_threshold)? {
                    found = true;
                }
