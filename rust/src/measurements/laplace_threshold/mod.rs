...
use crate::traits::samplers::LaplaceDist;
...
{
    use crate::traits::samplers::PSRN;
        ...
        Function::new_fallible(move |data: &HashMap<TK, TV>| {
            data.clone()
                ...
                .map(|(key, value)| {
                    ...
                    let mut psrn = LaplaceDist::new_psrn(f_value, f_scale.clone())?;
                    let is_above = psrn.is_above(&f_threshold)?;
                    Ok((key, psrn, is_above))
                })
                // only keep keys with values above threshold
                .filter(|res| res.as_ref().map(|(_, _, above)| *above).unwrap_or(true))
                .map(|res: Fallible<(TK, PSRN<LaplaceDist>, bool)>| {
                    let (k, mut v, _) = res?;
                    Ok((k, v.value()?))
                })
