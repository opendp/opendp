...
pub trait NoisePrivacyMap<MI: Metric, MO: Measure>: Sample {
    ...
    /// Otherwise if the output is `Ok(privacy_map)`
    /// then `privacy_map` observes the following:
    ///
    /// Under the condition that:
    /// * `input_metric` is `MI::default()`
    /// * `output_metric` is `MO::default()`
    /// * `function(x) = x + Z` where `Z` is a vector of iid samples from `self`
    ///
