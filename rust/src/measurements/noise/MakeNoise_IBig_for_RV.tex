...
\begin{proof}[Proof of privacy guarantee]
    ...
    Line~\ref{line:sample} calls \texttt{self.sample(x)} on the input vector.
    ...
    or \texttt{shift + Z} where \texttt{Z} is a vector sample from the distribution defined by \texttt{self}.
