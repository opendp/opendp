...
use crate::measures::{MultiDP, PrivacyGuarantee};
use crate::{
...
impl NoisePrivacyMap<L1Distance<RBig>, MultiDP> for ZExpFamily<1> {
    fn noise_privacy_map(
        ...
        Ok(PrivacyMap::new_fallible(move |d_in: &RBig| {
            ...
            if d_in.is_zero() {
                return PrivacyGuarantee::new()
                    .with_approxDP(vec![(0.0, 0.0)])?
                    .with_zCDP(0.0, 0.0)?
                    .with_renyiDP_trusted(move |_alpha| Ok(0.0), 0.0);
            }
            ...
            if !sensitivity.is_finite() {
                return fallible!(
                    FailedMap,
                    "sensitivity ({sensitivity}) must be finite for MultiDP"
                );
            }
            if !scale_f.is_finite() || scale_f <= 0.0 {
                return fallible!(
                    FailedMap,
                    "scale ({scale_f}) must be finite and positive for MultiDP"
                );
            }
