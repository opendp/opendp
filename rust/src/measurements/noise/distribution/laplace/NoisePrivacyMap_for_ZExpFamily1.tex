...
\title{\texttt{NoisePrivacyMap<L1Distance<RBig>, PureDP> for ZExpFamily<1>}}
...
\begin{itemize}
    ...
    \item \texttt{MO}, the output measure, is of type \rustdoc{measures/struct}{PureDP}
\end{itemize}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &=\max_{x \sim x'} \max_{S \subseteq \textrm{supp}(M(\cdot))} \Big[\ln \dfrac{\Pr[M(x) \in S]}{\Pr[M(x') \in S]} \Big]
            &&\text{substitute } \rustdoc{measures/struct}{PureDP} \\
