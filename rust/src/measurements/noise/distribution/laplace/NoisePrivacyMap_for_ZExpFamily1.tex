...
\begin{align}
    ...
    &=\max_{x \sim x'} \max_{y \in \mathbb{Z}^d_m} \ln \dfrac{\sum_{z \in (m\mathbb{Z}^d + y)} \exp(\sum_{i=1}^d -|x_i - z_i|/s)}{\sum_{z \in (m\mathbb{Z}^d + y)} \exp(\sum_{i=1}^d -|x'_i-z_i|/s)}
        &&\text{eliminate shared terms}
    % I feel the numerator and denominator should each simplify into something like \Pr[M(x) = y] / (1 - q^d), a la geometric series,
    % and then the extra terms cancel out, leaving you with a ratio of the non-modular probabilities. 
    % Then the proof continues as in the non-modular setting.
\end{align}
