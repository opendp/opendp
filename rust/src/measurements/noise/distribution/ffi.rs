...
use crate::measurements::noise::distribution::NoiseDistribution;
...
pub extern "C" fn opendp_measurements__make_noise(
    ...
    match (output_type.id, distribution) {
        ...
        (id, None) if id == Type::of::<MultiDP>().id => {
            if metric_type.descriptor.contains("L1Distance") {
                run_laplace()
            } else if metric_type.descriptor.contains("L2Distance") {
                run_gaussian()
            } else {
                err!(
                    FFI,
