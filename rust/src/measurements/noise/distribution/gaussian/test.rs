...
#[test]
fn test_make_noise_zexpfamily2_privacy_guarantee_matches_zcdp() -> Fallible<()> {
    let space = (AtomDomain::<IBig>::default(), AbsoluteDistance::default());
    let distribution = ZExpFamily::<2> { scale: rbig!(2) };

    let meas = distribution.make_noise(space, MultiDP)?;
    let curve = meas.map(&rbig!(2))?;
    let expected = PrivacyGuarantee::new().with_zCDP(gaussian_zcdp_upper(2., 2.)?, 0.0)?;
