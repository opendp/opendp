...
fn test_all() -> Fallible<()> {
    macro_rules! test_gaussian_with_ty {
        ($($ty:ty),+) => {$(
            let meas = make_gaussian(AtomDomain::<$ty>::default(), AbsoluteDistance::<$ty>::default(), 1., None)?;
            meas.invoke(&<$ty>::zero())?;
            meas.map(&<$ty>::one())?;

            let meas = make_gaussian(VectorDomain::new(AtomDomain::<$ty>::default()), L2Distance::<$ty>::default(), 1., None)?;
