...
use crate::{
    ...
    measures::{MultiDP, PrivacyGuarantee, zCDP},
    metrics::L2Distance,
...
impl NoisePrivacyMap<L2Distance<RBig>, MultiDP> for ZExpFamily<2> {
    fn noise_privacy_map(
        ...
        Ok(PrivacyMap::new_fallible(move |d_in: &RBig| {
            ...
            if d_in == &RBig::ZERO {
                return Ok(PrivacyGuarantee::new().with_zCDP(0.0, 0.0)?);
            }
            ...
            Ok(PrivacyGuarantee::new().with_zCDP(gaussian_zcdp_upper(sensitivity, sigma)?, 0.0)?)
        }))
    }
}
