...
\begin{proof}[Proof of privacy guarantee]
    ...
    Line~\ref{line:sample} calls \texttt{self.sample([v])} and projects the first value.
    ...
    the postcondition claims that either returns an error independently of the input singleton vector,
    or \texttt{[v] + Z} where \texttt{Z} is a length-one vector sample from the distribution defined by \texttt{self}.
