...
use crate::{
    ...
    measurements::{
        ...
        nature::float::{
            FloatExpFamily, find_nearest_multiple_of_2k, get_rounding_distance, integerize_scale,
            x_mul_2k,
        },
    },
...
impl<MO: 'static + Measure, TK, TV, const P: usize, QI: Number>
    ...
    ZExpFamily<P>: NoiseThresholdPrivacyMap<L0PI<P, AbsoluteDistance<RBig>>, MO>,
{
    ...
    > {
        ...
        let Ok(threshold) = RBig::try_from(threshold) else {
            return fallible!(FailedFunction, "threshold ({threshold}) must be finite");
        };

        let threshold = x_mul_2k(threshold, -k).round().into_parts().1;

        let t_int = make_float_to_bigint_threshold::<TK, TV, P, QI>(input_space, k)?;
    ...
    i32: ExactIntCast<TV::Bits>,
{
    ...
    Transformation::new(
        ...
        StabilityMap::new_fallible(move |(l0, lp, li): &(u32, QI, QI)| {
            let rounding_distance = get_rounding_distance::<TV, P>(k, Some(*l0 as usize))?;
            let lp = RBig::try_from(lp.clone())
                .map_err(|_| err!(FailedMap, "l{P} ({lp:?}) must be finite"))?;
            let lp = x_mul_2k(lp + rounding_distance.clone(), -k);

            let li = RBig::try_from(li.clone())
                .map_err(|_| err!(FailedMap, "li ({li:?}) must be finite"))?;
            let li = x_mul_2k(li + rounding_distance.clone(), -k);
            Ok((*l0, lp, li))
        }),
    )
}
