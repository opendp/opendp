...
\contrib
Proves soundness of the implementation of \rustdoc{measurements/noise_threshold/distribution/laplace/fn}{make\_laplace\_threshold} in \asOfCommit{mod.rs}{f5bb719}.
...
which constructs the core mechanism and wraps it in pre-processing transformations and postprocessors to match the desired parameterization.
...
\begin{itemize}
    ...
    \item generic \texttt{DiscreteLaplace} implements trait \rustdoc{measurements/noise_threshold/trait}{MakeNoiseThreshold}
...
\lstinputlisting[language=Python,firstline=2,escapechar=|]{./pseudocode/make_laplace_threshold.py}

\subsection*{Postcondition}
\begin{theorem}
    \validMeasurement{(\texttt{input\_domain, input\_metric, scale, threshold, k, DI, MI, MO})}{\texttt{make\_laplace\_threshold}}
\end{theorem}
