...
\title{\texttt{NoiseThresholdPrivacyMap<L01I<AbsoluteDistance<RBig>>, Approximate<MaxDivergence>> for ZExpFamily<2>}}
...
\begin{itemize}
    \item \texttt{MI}, the input metric, is of type \rustdoc{metrics/type}{L01I}\texttt{<AbsoluteDistance<RBig>>}
...
\begin{theorem}
    ...
    and discards pairs where $v_i + Z_i$ is less than \texttt{threshold}.
...
\begin{proof}
    ...
    Two separate tail bounds are computed, one being a bound based on the discrete laplace,
    and the other based on the continuous laplace.
    Two bounds are used because the discrete mass becomes more numerically unstable and difficult to compute, as $k$ gts smaller.
