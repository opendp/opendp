...
pub trait NoiseThresholdPrivacyMap<MI: Metric, MO: Measure>: Sample {
    ...
    /// and discards pairs where `v_i + Z_i` is less than `threshold`.
    ...
    ZExpFamily<P>: NoiseThresholdPrivacyMap<L0PI<P, AbsoluteDistance<RBig>>, MO>,
{
    ...
    > {
        ...
        let privacy_map =
            self.noise_threshold_privacy_map(&input_metric, &output_measure, threshold.clone())?;

        Measurement::new(
            input_domain,
            Function::new_fallible(move |data: &HashMap<TK, IBig>| {
                let threshold = threshold.as_ibig().clone();
                data.into_iter()
                    // noise output count
                    .map(|(k, v)| Ok((k.clone(), self.sample(v)?)))
                    // only keep keys with values gte threshold, and errors
                    .filter(|res| res.as_ref().map(|p| p.1 >= threshold).unwrap_or(true))
