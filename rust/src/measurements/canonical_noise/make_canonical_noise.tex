...
\texttt{d\_out} is passed in directly as a \texttt{PrivacyGuarantee}, which defines the tradeoff function via evaluation.
By the implementation of the fixed-point search,
\texttt{tradeoff} is a symmetric nontrivial tradeoff function
...
Since \texttt{tradeoff} is obtained by querying the tradeoff-function view of the
\texttt{PrivacyGuarantee} \texttt{d\_out}, the mechanism satisfies the f-DP
bound used by \texttt{MultiDP} for \texttt{d\_out} when input datasets differ
by exactly \texttt{d\_in}.

This guarantee is then reflected in the privacy map on line~\ref{privacy-map}.
If \texttt{d\_in\_p} does not equal \texttt{d\_in}, then the privacy map returns an error.
If \texttt{d\_in\_p} equals \texttt{d\_in} and \texttt{d\_in = 0}, then the privacy loss is a tradeoff curve that always returns 1.0.
Otherwise, if \texttt{d\_in\_p} equals \texttt{d\_in} and \texttt{d\_in > 0}, then the privacy loss is \texttt{d\_out}.
