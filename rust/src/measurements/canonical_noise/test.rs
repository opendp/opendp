...
fn test_canonical_noise_approxdp() -> Fallible<()> {
    let d_out = PrivacyGuarantee::new().with_approxDP(vec![(0.1, 1e-7)])?;
...
fn test_canonical_noise_fdp_exact_d_in() -> Fallible<()> {
    ...
    let mapped_partial = m_cnd.map(&0.5)?;
    assert_eq!(mapped_partial.beta(0.1)?, d_out.beta(0.1)?);

    let mapped_zero = m_cnd.map(&0.0)?;
    assert_eq!(mapped_zero.beta(0.0)?, 1.0);
    assert_eq!(mapped_zero.beta(1.0)?, 0.0);

    assert!(m_cnd.map(&-0.1).is_err());
    assert!(m_cnd.map(&1.1).is_err());
