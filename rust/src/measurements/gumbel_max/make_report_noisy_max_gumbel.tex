...
(where each candidate has zero probability, except for the true answer with probability one), 
...
In practice, computing $e^{s_i / \tau}$ is prone to 
...
\begin{theorem} 
    \label{gumbel-equiv} 
Sampling from K is equivalent to sampling from the distribution derived by computing the softmax function of the score vector, because $P(K=k) = p_k$. \cite{Medina2020DuffAD} 
Recall the softmax function $\sigma(z_i) = e^{z_i} / \sum_j e^{z_j}$,
which is equivalent to the probability distribution defined by the exponential mechanism on a discrete support.
\end{theorem} 
\begin{align*} 
    P(K = k) &= P(G_k = \max_i G_i) &&\text{by definition of K} \\ 
    &= P(-log(Z_k / N) = \max_i(-\ln(Z_i / N))) &&\text{by \ref{g-z-equiv}}\\ 
    &= P(log(Z_k / N) = \min_i(\ln(Z_i / N))) &&\text{since } max -a_i = -min_i a_i \\ 
...
It is shown in Section~\ref{privacy-guarantee} that a tighter analysis of the exponential mechanism is compatible with a score vector whose sensitivity is expressed in terms of this metric. 
...
\begin{itemize} 
    ...
    \item \texttt{f64} is a type with trait \rustdoc{traits/trait}{DistanceConstant}\texttt{<TIA>}
    \item \texttt{FBig} is a type that supports fallible exact casting from \texttt{TIA} and \texttt{f64}
\end{itemize} 
...
\validMeasurement{\texttt{input\_domain, input\_metric, scale, optimize, TIA}}{\texttt{make\_report\_noisy\_max\_gumbel}}  
 
\section{Proof} 

\subsection{Data-independent runtime errors.}
The only source of errors comes from \texttt{greater\_than}, which can in turn only occur due to lack of system entropy.
...
\begin{proof} 
    ...
    This is because an elementwise negation transformation applied to the score vector is 1-stable with respect to $L_\infty$ sensitivity.
...
\begin{align*} 
    ...
    &\le \ln\left(\frac{\exp(\frac{\epsilon \max_j(x'_j - x_j)}{\Delta}) \sum_{i} \exp(\frac{\epsilon x_i}{\Delta})}{\sum_{i} \exp(\frac{\epsilon x_i}{\Delta})}\right) 
        &&\begin{split}\text{by replacing each } x'_i-x_i \\ \text{with the greatest difference}\end{split}\\ 
...
\begin{align*} 
    ...
    &= \max_{x \sim x'} \max_i \ln\left(\frac 
        ...
        \right) &&\text{by Lemma~\ref{function-prob}, substitute \function}\\ 
    &= \max_{x \sim x'} \max_i \ln \left( 
        ...
        &&\text{by Section~\ref{gumbel-reparam}} \\ 
    &= \max_{x \sim x'} \max_i \ln \left(\frac 
    ...
    &\leq \max_{x \sim x'} \frac{\max_i (x_i - x'_i)}{\tau} + \frac{\max_j (x'_j - x_j)}{\tau} &&\text{by Lemma~\ref{priv-inequality}} \\ 
...
the inequality above shows that $\function(x)$ and $\function(x')$ are \dout-close under \texttt{output\_measure}  
