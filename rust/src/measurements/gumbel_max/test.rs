...
fn test_max_vs_min() -> Fallible<()> {
    assert_eq!(
        select_score([1, 2, 3].into_iter(), Optimize::Max, RBig::ZERO)?,
        2
    );
    assert_eq!(
        select_score([1, 2, 3].into_iter(), Optimize::Min, RBig::ZERO)?,
        0
    );

    assert_eq!(
        select_score([1, 1, 100_000].into_iter(), Optimize::Max, RBig::ONE)?,
        2
    );
    assert_eq!(
        select_score([1, 100_000, 100_000].into_iter(), Optimize::Min, RBig::ONE)?,
        0
    );
    Ok(())
}
