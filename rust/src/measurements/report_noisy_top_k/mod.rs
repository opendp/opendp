    ...
    f64: InfCast<TIA>,
{
    if input_domain.element_domain.nullable() {
        return fallible!(MakeMeasurement, "input_domain must not allow null values");
    }
    ...
    Measurement::new(
        input_domain,
        if scale.is_zero() {
            Function::new_fallible(function_report_top_k(k, optimize))
        } else {
            Function::new_fallible(function_report_noisy_top_k::<MO, _>(k, f_scale, optimize))
        },
        input_metric.clone(),
...
///
/// The returned function will only error if the score vector is empty.
fn function_report_top_k<TIA: Number>(
    k: usize,
    optimize: Optimize,
) -> impl Fn(&Vec<TIA>) -> Fallible<Vec<usize>> {
    ...
    move |x: &Vec<TIA>| {
        ...
        Ok(x_top.into_iter().map(|(i, _)| i).collect())
    }
}
...
fn function_report_noisy_top_k<MO: SelectionMeasure, TIA: Number>(
    k: usize,
    scale: FBig,
    optimize: Optimize,
) -> impl Fn(&Vec<TIA>) -> Fallible<Vec<usize>>
    ...
    FBig: TryFrom<TIA>,
{
    move |x: &Vec<TIA>| {
        let iter = (x.iter().enumerate())
            ...
            .map(|(i, shift)| {
                let rv = MO::random_variable(shift, scale.clone());
