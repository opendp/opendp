...
use crate::measurements::{make_gaussian, make_laplace, Gaussian, Laplace, MakeNoise};
    ...
    (VectorDomain<AtomDomain<f64>>, MO::Metric): MetricSpace,
{
    ...
    let dtype = middle_domain.active_series()?.field.dtype.clone();
    let support = match &dtype {
...
where
    ...
    Laplace: MakeNoise<VectorDomain<AtomDomain<T>>, L1Distance<f64>, MaxDivergence>,
    Gaussian: MakeNoise<VectorDomain<AtomDomain<T>>, L2Distance<f64>, ZeroConcentratedDivergence>,
    RBig: TryFrom<T>,
{
    let domain = VectorDomain::new(AtomDomain::<T>::default());
    ...
    Ok(ChunkedArray::try_from_chunk_iter(series.name().into(), chunk_iter)?.into_series())
}
