...
fn test_make_count_expr_grouped() -> Fallible<()> {
    ...
    let m_lap = len().make_private(expr_domain, L0PInfDistance(SymmetricDistance), PureDP, None)?;
...
fn test_make_count_expr_no_length() -> Fallible<()> {
    ...
    let variant = len()
        .make_private(expr_domain, L0PInfDistance(SymmetricDistance), PureDP, None)
        .map(|_| ())
        .unwrap_err()
        .variant;
