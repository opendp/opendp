...
use crate::{
    ...
    domains::{ExprDomain, ExprPlan, MarginPub},
    error::Fallible,
...
impl<M: 'static + UnboundedMetric> PrivateExpr<PartitionDistance<M>, MaxDivergence> for Expr {
    fn make_private(
        ...
        global_scale: Option<f64>,
    ) -> Fallible<Measurement<ExprDomain, ExprPlan, PartitionDistance<M>, MaxDivergence>> {
    ...
    for Expr
{
    fn make_private(
        ...
        global_scale: Option<f64>,
    ) -> Fallible<Measurement<ExprDomain, ExprPlan, PartitionDistance<M>, ZeroConcentratedDivergence>>
    {
        if expr_noise::match_noise_shim(&self)?.is_some() {
            return expr_noise::make_expr_noise(input_domain, input_metric, self, global_scale);
        }
    ...
    Expr: PrivateExpr<PartitionDistance<MI>, MO>,
{
    fn make_private(
        ...
        global_scale: Option<f64>,
    ) -> Fallible<Measurement<ExprDomain, ExprPlan, PartitionDistance<MI>, Approximate<MO>>> {
