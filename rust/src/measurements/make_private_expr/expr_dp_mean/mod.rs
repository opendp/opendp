...
use polars::{
    ...
    prelude::{AnonymousColumnsUdf, Column, ColumnsUdf, Expr, Operator, lit},
};
...
impl AnonymousColumnsUdf for DPMeanShim {
    ...
    fn get_field(
        ...
        fields
            .first()
            .cloned()
            .ok_or_else(|| polars_err!(InvalidOperation: "{} expects at least one column", Self::NAME))
    }
}
