...
use crate::traits::{InfCast, Integer};
...
pub fn make_baseline_toeplitz<T>(
    ...
    Measurement<VectorDomain<AtomDomain<T>>, Vec<T>, L1Distance<T>, ZeroConcentratedDivergence>,
>
where
...
///
/// # Returns
/// A Measurement that computes differentially private prefix sums of counts with a
/// zero-concentrated differential privacy (zCDP) guarantee from discrete Gaussian noise.
///
/// # Example
/// ```
/// # use opendp::prelude::*;
    ...
    f64: InfCast<T>,
{
    ...
    let n = match input_domain.size {
        ...
        None => {
            return fallible!(
                MakeMeasurement,
                "input domain must have known size for Toeplitz mechanism"
            );
        }
    };
...
mod tests {
    ...
    fn test_privacy_map() -> Fallible<()> {
        ...
        assert!((measurement.map(&5)? - 0.125).abs() < f64::EPSILON); // rho = (5/10)^2 / 2
        assert!((measurement.map(&20)? - 2.0).abs() < f64::EPSILON); // rho = (20/10)^2 / 2
        Ok(())
    }
