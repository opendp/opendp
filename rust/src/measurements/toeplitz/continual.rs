...
/// 4. The `privacy_cost` method calculates the privacy cost (rho) for a given sensitivity under zero-concentrated differential privacy (zCDP).
    ...
    T: Integer + Display + FromStr + CheckedAdd + CheckedMul + CheckedSub + Zero + num::One + PartialOrd + Clone,
{
        ...
        f64: InfCast<T>,
    {
        ...
        Ok((d_in_f64 / self.config.scale).powi(2) / 2.0)
    }
}
...
mod test {
    ...
    fn test_privacy_cost() -> Fallible<()> {
        ...
        assert!((mechanism.privacy_cost(1)? - 0.005).abs() < f64::EPSILON);
        assert!((mechanism.privacy_cost(10)? - 0.5).abs() < f64::EPSILON);
        assert!((mechanism.privacy_cost(20)? - 2.0).abs() < f64::EPSILON);
