...
Proves soundness of \rustdoc{measurements/fn}{make\_vector\_integer\_laplace} in \asOfCommit{mod.rs}{f5bb719}.
...
\lstinputlisting[language=Python,firstline=2,escapechar=|]{./pseudocode/make_vector_integer_laplace.py}

\subsection*{Postcondition}

\validMeasurement{\texttt{(input\_domain, input\_metric, scale, T, QO)}}{\\\texttt{make\_vector\_integer\_laplace}}
...
\begin{align*}
    ...
    =& \max_{x \sim x'} \max_{z \in \mathbb{Z^d}} \sum_i^d \ln\left(\frac{
    \exp \left( -\frac{|x_i - z_i|}{b} \right)
    }{
        \exp \left( -\frac{|x'_i - z_i|}{b} \right)
    }\right) &&\text{pull product out by log rules}\\
    =& \max_{x \sim x'} \max_{z \in \mathbb{Z^d}} \sum_i^d \frac{|x'_i - z_i| - |x_i - z_i|}{b}&& \text{exp and ln cancel} \\
