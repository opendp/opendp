...
fn test_max_vs_min_exponential() -> Fallible<()> {
    assert_eq!(
        select_score::<_, ExponentialRV>([1, 2, 3].into_iter(), Optimize::Max, FBig::ZERO)?,
        2
    );
    assert_eq!(
        select_score::<_, ExponentialRV>([1, 2, 3].into_iter(), Optimize::Min, FBig::ZERO)?,
        0
    );

    assert_eq!(
        select_score::<_, ExponentialRV>([1, 1, 100_000].into_iter(), Optimize::Max, FBig::ONE)?,
        2
    );
    assert_eq!(
        select_score::<_, ExponentialRV>(
