...
use crate::{
    ...
    traits::{DistanceConstant, ExactIntCast, Float, InfCast, Number},
    transformations::{
        make_quantile_score_candidates_expr, ARDatasetMetric, DQuantileOuterMetric, IntoFrac, ToVec,
    },
};
...
use super::then_discrete_exponential_expr;
...
pub fn make_private_quantile<MI, TIA, QO, A>(
    ...
    ChunkedArray<TIA::Polars>: ToVec<TIA>,
{
    make_quantile_score_candidates_expr(input_domain, input_metric, candidates.clone(), alpha)?
        >> then_discrete_exponential_expr(temperature, Optimize::Min)
...
mod test_make_discrete_quantile {
    ...
    fn test_discrete_dp_quantile_select() -> Fallible<()> {
        ...
        let meas = make_private_quantile(expr_domain, InsertDeleteDistance, candidates, 1., 0.8)?;
    ...
    fn test_discrete_dp_quantile_groupby() -> Fallible<()> {
        ...
        // Get resulting index (expression result)
        let meas =
            make_private_quantile(expr_domain, Lp(InsertDeleteDistance), candidates, 1., 0.5)?;
