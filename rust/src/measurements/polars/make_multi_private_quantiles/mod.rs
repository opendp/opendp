    ...
    ChunkedArray<TIA::Polars>: ToVec<TIA>,
{
    ...
    Measurement::new(
        ...
        Function::new_expr(move |expr: Expr| -> Expr {
            let shared_temperature = temperature/(LOG2_E * (nb_alphas as f64) + 1.0);
            impl_function(input_domain, input_metric, candidates, expr, alphas, bounds, shared_temperature)
        }),
        input_metric,
    ...
    ChunkedArray<TIA::Polars>: ToVec<TIA>,
{
    ...
    if nb_alphas == 1 {
        return make_private_quantile(input_domain, input_metric, candidates, temperature, alphas.0)?.function
    }
    ...
    let p = alphas[mid];
    let v = 
        make_private_quantile(input_domain, input_metric, candidates, temperature, p)?.function;
    ...
    return [
        impl_function(input_domain, input_metric, candidates, expr, alphas_l, (bounds.0, v), temperature),
        v,
        impl_function(input_domain, input_metric, candidates, expr, alphas_u, (v, bounds.1), temperature)
    ]
}
...
mod test_make_multi_discrete_quantile {
    ...
    fn test_discrete_dp_multi_quantiles_select() -> Fallible<()> {
        ...
        let meas = make_multi_private_quantile(expr_domain, InsertDeleteDistance, candidates, 1., 3)?;
    ...
    fn test_discrete_dp_mutli_quantiles_groupby() -> Fallible<()> {
        ...
        // Get resulting index (expression result)
        let meas =
        make_multi_private_quantile(expr_domain, Lp(InsertDeleteDistance), candidates, 1., 3)?;
