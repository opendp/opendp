...
use crate::accuracy::tail_bounds::integrate_discrete_noise_tail;
    ...
    (ExprDomain, PartitionDistance<MI>): MetricSpace,
{
    ...
    let threshold = if margin.public_info.is_some() {
        None
    } else if let Some((name, threshold)) = expr_threshold {
        let noise = find_len_expr(&dp_exprs, Some(name.as_str()))?.1;
        Some((name, noise, threshold))
    ...
    let m_group_by_agg = Measurement::new(
        ...
        PrivacyMap::new_fallible(move |&d_in| {
            ...
            if let Some((_, noise, threshold)) = &threshold {
                let distribution = noise.distribution.expect("distribution is known");
                let scale = noise.scale.expect("scale is known");

                let d_instability = f64::inf_cast(threshold.inf_sub(&li)?)?;
                let delta_p = integrate_discrete_noise_tail(distribution, scale, d_instability)?;
                d_out = MO::add_delta(d_out, delta_p)?;
