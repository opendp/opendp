...
use crate::{
    ...
    metrics::{
        ...
        InsertDeleteDistance, L01InfDistance, MicrodataMetric, SymmetricDistance,
        SymmetricIdDistance,
    },
...
#[bootstrap(features("contrib"), returns(c_type = "FfiResult<AnyMeasurement *>"))]
#[unsafe(no_mangle)]
pub extern "C" fn opendp_measurements__make_private_database_lazyframe(
    ...
    let input_domain = try_!(try_as_ref!(input_domain).downcast_ref::<DatabaseDomain>()).clone();
    let input_metric =
        try_!(try_as_ref!(input_metric).downcast_ref::<DatabaseIdDistance>()).clone();
    let output_measure = try_as_ref!(output_measure);
    let MO_ = output_measure.type_.clone();
    let lazyframe = try_!(try_as_ref!(lazyframe).downcast_ref::<LazyFrame>()).clone();
    ...
    fn monomorphize<
        MO: 'static + Measure + crate::combinators::CompositionMeasure + ApproximateMeasure,
    >(
    ...
    where
        polars::prelude::Expr: crate::measurements::PrivateExpr<
            ...
            >,
        DslPlan:
            StableDslPlan<DatabaseDomain, DatabaseIdDistance, FrameDistance<SymmetricDistance>>,
