...
/// * `output_measure` - One of `BoundedRange` or `MaxDivergence`.
...
pub fn make_report_noisy_max<MO: ArgmaxMeasure, TIA>(
    ...
    f64: InfCast<TIA>,
{
    ...
    Measurement::new(
        ...
        Function::new_fallible(move |arg: &Vec<TIA>| {
            select_score::<_, MO::RV>(arg.iter().cloned(), optimize.clone(), f_scale.clone())
        }),
        input_metric.clone(),
        output_measure,
...
pub trait ArgmaxMeasure: 'static + Measure<Distance = f64> {
    type RV: ArgmaxRV;
}

impl ArgmaxMeasure for BoundedRange {
    type RV = GumbelRV;
}

impl ArgmaxMeasure for MaxDivergence {
...
pub trait ArgmaxRV: InverseCDF + Sized {
...
impl ArgmaxRV for ExponentialRV {
    fn new(shift: FBig, scale: FBig) -> Fallible<Self> {
