...
use crate::measures::PrivacyGuarantee;
...
pub extern "C" fn opendp_data__object_as_slice(obj: *const AnyObject) -> FfiResult<*mut FfiSlice> {
    ...
    fn tuple_curve_f64_to_raw(obj: &AnyObject) -> Fallible<FfiSlice> {
        let (curve, delta) = obj.downcast_ref::<(PrivacyGuarantee, f64)>()?;
    ...
    match &obj.type_.contents {
        ...
        _ => { dispatch!(plain_to_raw, [(obj.type_, [u8, u16, u32, u64, u128, i8, i16, i32, i64, i128, usize, f32, f64, bool, PrivacyGuarantee, AnyMeasurement, AnyQueryable])], (obj)) }
    }.into()
}
...
pub extern "C" fn ffiresult_err(
    ...
    fn make_message(message: *const c_char, backtrace: *const c_char) -> Fallible<*mut c_char> {
    ...
    let variant = util::to_str(variant)
        .ok()
        .and_then(|variant| variant.parse::<ErrorVariant>().ok())
        .unwrap_or(ErrorVariant::FFI)
        .to_string();
