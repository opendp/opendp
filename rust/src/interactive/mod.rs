...
impl Wrapper {
    ...
    pub fn new_recursive_pre_hook(
        hook: impl FnMut() -> Fallible<()> + Send + Sync + 'static,
    ) -> Wrapper {
    ...
    Self: IntoPolyQueryable + FromPolyQueryable,
{
    ...
    pub(crate) fn new_interactive(
        transition: impl FnMut(&Self, Query<Q>) -> Fallible<Answer<A>> + 'static + Send + Sync,
        wrapper: Option<Wrapper>,
    ) -> Fallible<Self> {
        let queryable = Queryable::new(transition);
        Ok(match wrapper {
            None => queryable,
            Some(wrap) => Queryable::from_poly(wrap(queryable.into_poly())?),
        })
    }
