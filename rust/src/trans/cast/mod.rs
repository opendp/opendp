...
use crate::dist::{IntDistance, SubstituteDistance, SymmetricDistance};
...
pub fn make_cast_metric<D, MI, MO>(domain: D) -> Fallible<Transformation<D, D, MI, MO>>
where
    D: Domain + Clone,
    D::Carrier: Clone,
    MI: DatasetMetric,
...
mod tests {
    ...
    fn test_cast_metric() -> Fallible<()> {
        let data = vec!["abc".to_string(), "1".to_string(), "1.".to_string()];
        let caster =
            make_cast_metric::<VectorDomain<AllDomain<_>>, SubstituteDistance, SymmetricDistance>(
