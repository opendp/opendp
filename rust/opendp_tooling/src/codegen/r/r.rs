...
pub(crate) fn generate_r_function(
    ...
    let then_func = if func.name.starts_with("make_") {
        ...
        let constructor_args = if func.supports_partial {
            let domain_name = sanitize_r(func.args[0].name(), func.args[0].is_type);
            let metric_name = sanitize_r(func.args[1].name(), func.args[1].is_type);
            vec![
                format!("{domain_name} = output_domain(base_lhs)"),
                format!("{metric_name} = output_metric(base_lhs)"),
            ]
        } else {
            vec![]
        }
