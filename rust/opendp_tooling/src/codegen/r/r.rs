...
static ATOM_TYPES: &'static [&'static str] = &["u32", "u64", "i32", "i64", "f32", "f64", "usize", "bool", "String"];
...
pub fn generate_r_module(
    ...
    format!("# Auto-generated. Do not edit.
...
{args}),
        ...
        log)
            ...
            then_args = indent(once("lhs".to_string()).chain(args[offset..].to_owned()).collect::<Vec<_>>().join(",\n")),
            then_log = indent(generate_logger(module_name, func, true)),
            name = func.name,
            args = indent(indent(indent(
                if func.supports_partial { vec!["output_domain(lhs)".to_string(), "output_metric(lhs)".to_string()] } else { vec![] }.into_iter().chain(
                func.args[offset..]
                    .iter()
                    .map(|arg| {
...
fn generate_logger(module_name: &str, func: &Function, then: bool) -> String {
    let func_name = if then {func.name.replacen("make_", "then_", 1)} else {func.name.clone()};
    let offset = if then && func.supports_partial { 2 } else { 0 };
    let keys = func.args[offset..].iter().map(|arg| {
        format!("\"{}\"", arg.name())
    }).collect::<Vec<_>>().join(", ");

    let vals = func.args[offset..].iter().map(|arg| {
