...
use crate::{codegen::{indent, r::BLACKLIST}, Argument, Function, TypeRecipe};
...
fn generate_data_converter(func: &Function) -> String {
    let protect = flatten_args_for_c(func).iter().map(|arg| {
        format!("PROTECT({});", arg.name())
    }).collect::<Vec<_>>().join("\n");
    let data_converter: String = (func.args.iter())
        .map(|arg| {
            ...
            format!(r#"{c_type} c_{name} = {converter};"#, 
                c_type=arg.c_type(), 
                name=arg.name(), 
...
fn c_to_r(arg: Argument) -> String {
    ...
    if arg.c_type().starts_with("FfiResult<") {
        ...
        (inner.c_type.as_mut())
            .map(|ct| *ct = ct["FfiResult<".len()..ct.len() - 1].to_string());
...
fn flatten_args_for_c(function: &Function) -> Vec<Argument> {
    ...
    let arg_names = function.args.iter().map(|arg| arg.name().clone()).collect::<HashSet<_>>();
    args.extend(function.derived_types.iter().filter(|arg| !arg_names.contains(&arg.name())).cloned());

    args.extend((function.args.iter())
