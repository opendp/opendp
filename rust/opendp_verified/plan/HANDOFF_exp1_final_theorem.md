...
`rust/opendp_verified/src/samplers/bernoulli/exp1.lean`:
...
**Status as of end of 2026-06-30 session:** the tree builds **GREEN** (exit 0, 2866/2866
jobs, no `sorry` in `exp1.lean`). Increment 3 of
`rust/opendp_verified/src/samplers/bernoulli/exp1.lean` is fully proved. The **final end-to-end
theorem is not yet written** — that is the next task.

## Build / iterate

- Fast leaf rebuild (deps cached, ~10 s, only recompiles `exp1`):
  `cd <repo root> && lake build OpenDPVerified`  (exit 0 + zero `error:` = green)
- Full reliable verification (guard → cache → build):
  `cd <repo> && ./tools/build_lean.sh > /tmp/lb.log 2>&1`
...
`rust/opendp_verified/src/samplers/uniform/pmf.lean` → `samplerDist_loop_rejection_uniform`.
...
`rust/opendp_verified/src/samplers/uniform/pmf.lean` : `samplerDist_loop_rejection_uniform` (lines ~257–372)
