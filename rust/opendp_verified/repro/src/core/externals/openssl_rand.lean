...
def openssl.rand.UniformBytes (n : Nat) (buffer : Slice Std.U8) : Prop :=
  buffer.length = n ∧
  OpenDP.Core.Bytes.beBytesToNat buffer.val < OpenDP.Core.Bytes.byteRadix ^ n ∧
  OpenDP.Core.Bytes.uniformByteNatPMF n (OpenDP.Core.Bytes.beBytesToNat buffer.val) =
    (1 : ENNReal) / ((OpenDP.Core.Bytes.byteRadix ^ n : Nat) : ENNReal)
