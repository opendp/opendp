...
#
#   * repro `lean-toolchain`           (the canonical Lean version)
...
#   * Mathlib         (inputRev in the repro lake-manifest + its lean-toolchain)
#
...
# root (its own lean-toolchain + lake-manifest + .lake) is `repro/` = one up;
# the shared aeneas/SampCert checkouts live at the git repo root = three more up.
...
# 1. sub-package toolchain = the canonical version
check "lean-toolchain (repro)"     "$LEAN_TOOLCHAIN" "$(ttc "$proj_dir/lean-toolchain")"
...
# 6. Mathlib pin recorded in the sub-package manifest
