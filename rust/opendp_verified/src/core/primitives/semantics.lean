...
namespace OpenDP.Core.Semantics
        ...
        f (core.result.Result Nat error.Error)
          (do let pair ← samplers.fill_bytes buffer
              ...
              | core.result.Result.Ok _ =>
                  ok (core.result.Result.Ok (OpenDP.Core.Bytes.beBytesToNat pair.2.val))
              | core.result.Result.Err e =>
                  ok (core.result.Result.Err e))
          (core.result.Result.Ok k) =
        OpenDP.Core.Bytes.uniformByteNatPMF buffer.length k) ∧
...
theorem samplerDist_fill_bytes_nat (buffer : Slice Std.U8) :
    ...
    samplerDist
      (do let pair ← samplers.fill_bytes buffer
          ...
          | core.result.Result.Err e =>
              ok (core.result.Result.Err e)) =
    OpenDP.Core.Bytes.uniformByteNatPMF n := by
  funext k
...
theorem fill_bytes_nat_bridge
    ...
    ∑' pair : (core.result.Result Unit error.Error) × Slice Std.U8,
      ...
      (match pair.1 with
       | core.result.Result.Ok _ => g (OpenDP.Core.Bytes.beBytesToNat pair.2.val)
       | core.result.Result.Err _ => 0) =
    ∑' k : Nat, OpenDP.Core.Bytes.uniformByteNatPMF byte_len.val k * g k := by
  ...
  · -- Ok branch: the wrapper outputs `Ok (beBytesToNat s.val)`, point mass at that nat.
    ...
    rw [tsum_eq_single (OpenDP.Core.Bytes.beBytesToNat s.val) (fun k hk => by rw [if_neg hk]; ring)]
    rw [if_pos rfl]; ring
