...
| mark | meaning |
|------|---------|
| ✅ | proved here (`src/`), zero `sorry` |
| 🟡 | Rust **extracted** into `Generated/`, proof **not yet ported** here (a reference proof may exist in the superseded pre-rewrite tree — see the `lean` branch history) |
...
- **Rust:** `samplers.fill_bytes` · **Lean:** `OpenDP.Core.Bytes.uniformByteNatPMF`, `OpenDP.Core.Semantics.samplerDistGen_fill_bytes_nat` (the "hardware theorem"), `fill_bytes_nat_bridge`.
...
- **Lean:** `sample_uniform_ubig_below_pmf`, end-to-end `sample_uniform_ubig_below_spec` = `uniformNatBelowPMF`. The a14083a6 `deref_mut` loop-body reduction lives in `src/samplers/uniform/blockers.lean` (`body_eq`).

### 3. `sample_uniform_ubig_below` → `sample_bernoulli_fraction` ✅
Draw `k` uniform on `[0, denom)`; return `⊤` iff `k < numer` → `Bernoulli(numer/denom)`.
- **Rust:** `samplers.bernoulli.sample_bernoulli_rational` · **Lean:** `sample_bernoulli_rational_pmf`; `bernoulliPMF = SLang.BernoulliSamplePMF` grounds it against the trusted reference.
...
- **Lean:** `sample_bernoulli_exp1_spec` = `BernoulliExpNegSampleUnit` (`src/samplers/bernoulli/exp1.lean`), via `exp1_loop_cut_step` (cut-depth ↔ SampCert `BESL` loop), `exp1_loop_probWhile` (`⨆`/`tsum_iSup_commute` lift), and `probWhile_besl_eq_aux` (SampCert `..._sup`/`..._apply`) + `ℕ+`→`ℕ` reindex. One temporary axiom `div_rbig_by_ubig_exact_bernoulli_setup` (Dashu exact-division boundary).
...
- **Lean:** `sample_bernoulli_exp_spec` (`src/samplers/bernoulli/exp.lean`) — strong induction on
...
- **Lean:** `sample_geometric_exp_slow_spec` (`src/samplers/geometric/slow.lean`):
...
- **Lean:** `sample_geometric_exp_fast_spec` (`src/samplers/geometric/fast.lean`):
...
- **Lean:** `sample_discrete_laplace_spec` (`src/samplers/laplace.lean`):
...
- **Lean:** `sample_discrete_gaussian_spec` (`src/samplers/gaussian.lean`):
...
- The formal dependency graph for the ✅/🟡 stages is rendered by the blueprint
  (`tools/build_blueprint_lean.sh` → `rust/opendp_verified/blueprint/web/dep_graph_document.html`),
...
- **Completion is machine-checked**: `tools/build_lean.sh` ends with
  `tools/check_verified_chain_lean.sh`, which fails the build if any of the eight
