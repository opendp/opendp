...
pub extern "C" fn opendp_trans__make_bounded_mean(
    ...
    fn monomorphize<T>(
        ...
        where T: DistanceConstant + Sub<Output=T> + Float,
              for<'a> T: Sum<&'a T> {
        fn monomorphize2<MI, MO>(lower: MO::Distance, upper: MO::Distance, length: usize) -> FfiResult<*mut AnyTransformation>
            ...
            make_bounded_mean::<MI, MO>(lower, upper, length).into_any()
        }
