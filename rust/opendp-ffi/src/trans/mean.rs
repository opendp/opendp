...
pub extern "C" fn opendp_trans__make_sized_bounded_mean(
    ...
    fn monomorphize<T>(size: usize, lower: *const c_void, upper: *const c_void) -> FfiResult<*mut AnyTransformation>
        ...
        make_sized_bounded_mean::<T>(size, lower, upper).into_any()
    }
    let size = size as usize;
    let T = try_!(Type::try_from(T));
    dispatch!(monomorphize, [(T, @floats)], (size, lower, upper))
}
