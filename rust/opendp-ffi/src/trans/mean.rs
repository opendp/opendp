...
use opendp::err;
use opendp::traits::{DistanceConstant, InfCast, ExactIntCast, AlertingMul, CheckNull, AlertingSub, InfDiv};
...
pub extern "C" fn opendp_trans__make_sized_bounded_mean(
    ...
    fn monomorphize<T>(size: usize, bounds: *const AnyObject) -> FfiResult<*mut AnyTransformation>
        where T: DistanceConstant<IntDistance> + Sub<Output=T> + Float + ExactIntCast<usize> + AlertingMul + CheckNull + AlertingSub + InfDiv,
              for<'a> T: Sum<&'a T>,
