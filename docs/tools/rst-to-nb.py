...
def run_command(cmd, debug=False):
    ...
    if debug:
        print(f"RUN: {cmd}")
    completed_process = subprocess.run(cmd, capture_output=True, shell=True, text=True)
...
def clean_rst(rst_text, prefix):
    ...
    rst_text = sub(r"(:start-after: \S+)", rf"{spaces}\1", rst_text)
    rst_text = sub(r"(:end-before: \S+)", rf"{spaces}\1", rst_text)
...
def rst_to_md(dirty_rst_text, prefix, debug=False):  # pragma: no cover
    ...
    with text_to_temp(clean_rst_text, ".clean.rst", debug) as temp:
        return run_command(f"pandoc --from rst --to markdown {temp}", debug)
