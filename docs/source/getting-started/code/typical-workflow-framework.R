...
# mediate
data <- sample(n = 100L, k = 100L, replace = TRUE)
...
make_dp_count <- function(scale) {
  m_ac("input_space") |> then_count() |> then_laplace(scale)
}

scale <- binary_search_param(
  make_dp_count, d_in, d_out / 3L
)
...
sum_measurement <- binary_search_chain(
  function(scale) sum_transformation |> then_laplace(scale), d_in, d_out / 3L
)

dp_sum <- queryable(query = sum_measurement) # Different from Python, which does not require "query".
cat("dp_sum:", dp_sum, "\n")
# /sum
