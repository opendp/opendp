...
Owner-Claim Preservation
------------------------

The relational identifier metric is best understood as an owner-claim
preservation system.

Before truncation, every row must carry a valid owner claim. Operators
are stable when they preserve, refine, or soundly update that claim.
Joins union claims and deduplicate equivalent owner factors through
frame-local bindings. Truncation consumes the owner claim and converts
identifier-level reasoning into bounded event- or group-level distance.

This is the main simplification to keep in mind:

* the restriction is not "no aggregation before truncation"
* the restriction is "no owner-erasing operation before truncation"

That leads to a practical rule of thumb:

* allowed before truncation: operators that preserve ownership
* rejected before truncation: operators that erase ownership


Owner-Preserving Rollups
------------------------

Pre-truncation aggregation is valid when it preserves a complete owner
claim.
...
The grouped row still carries the owner claim ``[user_id]``, so the
rollup is owner-preserving.
...
retains a complete owner claim into the selected ``protect`` space.

By contrast, grouping by a visible key that erases the owner claim is
not directly allowed under the identifier metric. For example, if rows
are owned by ``user_id``, then grouping only by ``item`` loses the
protected owner.

That kind of query needs an owner-aware partial aggregation pipeline:
...
OpenDP currently supports the owner-preserving cases directly and
rejects owner-erasing group-by operations in the stable identifier
metric path.

This is why nested-table rollups can be valid before truncation:
aggregation itself is not the problem. Losing the owner claim is the
problem.
...
2. A join between two private branches that *both* carry the protected
   identifier. This is only allowed when the join aligns the active
   owner claim on both sides.
...
* Before truncation, every row must carry a valid owner claim.
* Pre-truncation aggregation is allowed when it preserves a complete
  owner claim.
* If two private branches both carry the protected identifier, the join
  must align the active owner claim on both sides.
