...
For this tutorial, we selected a few columns of interest from the public microdata of France across 10 study years. 
...
* ``privacy_unit``: The greatest influence an individual may have on your dataset.
  ...
  Alternatively, if you have a user identifier column ``PIDENT``, 
  you could set the privacy unit to ``dp.unit_of(contributions=1, identifier="PIDENT")``.
  For simplicity, examples will use the default privacy unit of 36 row contributions, 
  except when demonstrating how to use user identifiers.
