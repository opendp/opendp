...
.. doctest::
    ...
    >>> dp.enable_features('contrib') # we are using un-vetted constructors
    ...
    ...
    ...
    >>> transformation = (
    ...     dp.t.make_split_dataframe(',', col_names=['Student', 'Score']) >>
    ...     dp.t.make_select_column(key='Score', TOA=str) >>
    ...     dp.t.then_cast(TOA=float) >>
    ...     dp.t.then_impute_constant(constant=constant) >>
    ...     dp.t.then_clamp(bounds) >>
    ...     dp.t.then_resize(size, constant=constant) >>
    ...     dp.t.then_mean()
    ... )
...
.. note::
    ...
    * release a DP count first (:func:`count <opendp.transformations.make_count>` >> :func:`base_laplace <opendp.measurements.make_laplace>`), and then supply that count to resize
...
The :func:`base_laplace <opendp.measurements.make_laplace>` measurement qualifies.
...
.. doctest::
    ...
    >>> make_chain = lambda s: transformation >> dp.m.then_laplace(s)
    >>> scale = dp.binary_search_param(make_chain, d_in=num_tests, d_out=budget) # -> 1.33
