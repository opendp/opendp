...
It is recommended to install the "rust-analyzer" and "LaTex Workshop" extensions.

Some starter settings are in ``/.vscode/settings.default.json``, which can be copied to ``/.vscode/settings.json``.
With these settings, the ``Problems`` panel will show live compilation errors as you work.
These settings also configure LaTex Workshop to write .pdfs and auxiliary files to ``./out/``, which is ``.gitignored``.

A baseline configuration for ``/.vscode/tasks.json`` is provided in ``.vscode/tasks.default.json``. 
