...
def render_pairwise_visualization():
    ...
    pairwise_plot_path = Path(__file__).with_name("theil-sen-pairwise-fits.png")

    pairwise_plot_x_cuts = np.array([-1.5, 1.5])
    pairwise_plot_x = np.linspace(-2.5, 2.5, 12)
    pairwise_plot_y = 2 * pairwise_plot_x + 1
    pairwise_plot_data = np.column_stack([pairwise_plot_x, pairwise_plot_y])
    p1, p2 = np.array_split(pairwise_plot_data, 2)

    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.scatter(pairwise_plot_data[:, 0], pairwise_plot_data[:, 1], color="black", s=20)
    ...
    for x_cut in pairwise_plot_x_cuts:
        ax.axvline(x_cut, color="#2c4c7c", linestyle="--", linewidth=1)
...
def render_private_fit_visualization():
    ...
    private_fit_plot_path = Path(__file__).with_name("theil-sen-private-fit.png")

    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.scatter(x, y, color="black", s=14, alpha=0.7, label="synthetic data")

    plot_x = np.linspace(x.min(), x.max(), 200)
    ax.plot(plot_x, f(plot_x), color="#2c4c7c", linewidth=2, label="true fit")
    ax.plot(
        plot_x,
...
if __name__ == "__main__":
    render_pairwise_visualization()
    render_private_fit_visualization()
