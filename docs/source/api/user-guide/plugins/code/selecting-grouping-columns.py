...
def make_grouping_cols_score(candidates, min_bin_contributions):
    ...
    return dp.t.make_user_transformation(
        ...
        output_domain=dp.vector_domain(dp.atom_domain(T=float, nan=False)),
...
private_data = pd.DataFrame(
    {
        **{
            f"too_uniform_{n}": [random.randint(0, 1) for _ in range(row_count)]
            for n in range(col_count)
        },
        **{
            f"too_diverse_{n}": [random.randint(0, row_count) for _ in range(row_count)]
            for n in range(col_count)
        },
        **{
            f"just_right_{n}": [random.randint(0, 20) for _ in range(row_count)]
            for n in range(col_count)
        },
    }
)
