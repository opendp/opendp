...
rt_infer <- function(public_example, allow_extrinsic = FALSE) {
  ...
  if (is.list(public_example)) {
    return(new_runtime_type(
      "Tuple",
      lapply(public_example, rt_infer, allow_extrinsic = allow_extrinsic)
    ))
  }
  ...
  stop(
    "unable to infer type from object of class ",
    class(public_example)[1],
    call. = FALSE
  )
}
...
rt_canon <- function(type_name, generics = list()) {
  if (inherits(type_name, "runtime_type")) {
    type_args <- type_name$args
    if (!is.null(type_args)) {
      type_args <- lapply(type_args, rt_canon, generics = generics)
    }
    return(new_runtime_type(
