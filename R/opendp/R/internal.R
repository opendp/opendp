...
`_binary_search` <- function(
  ...
  .T <- parse_or_infer(type_name = .T, public_example = get_first_non_null(lower, upper))
  .T.predicate <- rt_canon("bool")
  .T.lower <- rt_canon(new_runtime_type(origin = "Option", args = list(.T)))
  .T.upper <- rt_canon(new_runtime_type(origin = "Option", args = list(.T)))

  log_ <- new_constructor_log("_binary_search", "internal", new_hashtab(
    list("predicate", "lower", "upper", "return_sign", "T"),
    list(predicate, lower, upper, unbox2(return_sign), .T)
  ))

  # Assert that arguments are correctly typed.
  rt_assert_is_similar(expected = .T.lower, inferred = rt_infer(lower))
  rt_assert_is_similar(expected = .T.upper, inferred = rt_infer(upper))
  rt_assert_is_similar(expected = "bool", inferred = rt_infer(return_sign))

  # Call wrapper function.
  output <- .Call(
    "internal___binary_search",
    predicate, lower, upper, return_sign, .T, .T.predicate, .T.lower, .T.upper,
    log_, PACKAGE = "opendp")
  output
}
