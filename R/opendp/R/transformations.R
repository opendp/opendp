...
make_count_by <- function(
  ...
  .TV <- rt_parse(type_name = .TV, generics = list(".MO"))
  .MO <- rt_substitute(.MO, .TV = .TV)
  .TV <- rt_substitute(.TV, .MO = .MO)
...
make_lipschitz_float_mul <- function(
  ...
  # Standardize type arguments.
  .D <- rt_parse(type_name = .D, generics = list(".M", ".T"))
  .M <- rt_parse(type_name = .M, generics = list(".D", ".T"))
  .T <- get_atom_or_infer(.D, constant)
  .D <- rt_substitute(.D, .M = .M, .T = .T)
  .M <- rt_substitute(.M, .D = .D, .T = .T)
  .T.bounds <- new_runtime_type(origin = "Tuple", args = list(.T, .T))
