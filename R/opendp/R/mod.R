...
new_privacy_guarantee_internal <- function(ptr) {
  privacy_guarantee <- function(attr, epsilon, delta, alpha, beta) {
    ...
    if (!missing(epsilon)) {
      return(`_privacy_guarantee_delta`(ptr, epsilon))
    }

    if (!missing(delta)) {
      return(`_privacy_guarantee_epsilon`(ptr, delta))
    }
...
# Deprecated compatibility constructor retained for callers of the former
# privacy-profile representation.
new_privacy_profile <- new_privacy_guarantee_internal

#' Construct a privacy guarantee.
#'
#' A privacy guarantee contains multiple simultaneously valid privacy
#' representations for the same mechanism and neighboring relation. Every
#' supplied representation holds conjunctively and may have different strength
#' or closure properties under later operations. Supply one or more of
#' `profile`, `log_profile`, `tradeoff`, `symmetric_tradeoff`, `approxDP`,
#' `gaussianDP`, `renyiDP`, or `zCDP`.
#'
#' @concept mod
#'
...
#' @param renyiDP_delta Source delta in OpenDP's additive approximate-RDP
#'   convention, interpreted only by the RDP conversion theorem.
#' @param zCDP zCDP parameter `rho`.
#' @param zCDP_delta Source delta in OpenDP's additive approximate-zCDP
#'   convention, interpreted only by the zCDP conversion theorem.
#'
...
privacy_guarantee <- function(
  ...
  if (!supplied[["renyiDP"]] && renyiDP_delta != 0) {
    stop("renyiDP_delta requires renyiDP", call. = FALSE)
  }
  if (!supplied[["zCDP"]] && zCDP_delta != 0) {
  ...
  privacy_guarantee_obj <- `_new_privacy_guarantee`()

  if (supplied[["profile"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_profile`(privacy_guarantee_obj, profile, log = FALSE)
  }

  if (supplied[["log_profile"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_profile`(privacy_guarantee_obj, log_profile, log = TRUE)
  }

  if (supplied[["tradeoff"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_tradeoff`(privacy_guarantee_obj, tradeoff, symmetric = FALSE)
  }

  if (supplied[["symmetric_tradeoff"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_tradeoff`(privacy_guarantee_obj, symmetric_tradeoff, symmetric = TRUE)
  }

  if (supplied[["approxDP"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_approxDP`(privacy_guarantee_obj, approxDP)
  }

  if (supplied[["gaussianDP"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_gaussianDP`(privacy_guarantee_obj, gaussianDP)
  }

  if (supplied[["renyiDP"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_renyiDP`(privacy_guarantee_obj, renyiDP, renyiDP_delta)
  }

  if (supplied[["zCDP"]]) {
    privacy_guarantee_obj <- `_privacy_guarantee_with_zCDP`(privacy_guarantee_obj, zCDP, zCDP_delta)
  }

  privacy_guarantee_obj
}
  ...
  tryCatch(
    ...
    error = function(e) {
      ...
      if (identical(rust_error$variant, "Search")) {
        error_message <- rust_error$message
        if (is.list(bounds) && length(bounds) == 2) {
          if (
            identical(error_message, "unable to infer upper bound") &&
              !is.null(bounds[[1]]) && is.null(bounds[[2]])
          ) {
            error_message <- "the decision boundary is below the lower bound"
          }
          if (
            identical(error_message, "unable to infer lower bound") &&
              is.null(bounds[[1]]) && !is.null(bounds[[2]])
          ) {
            error_message <- "the decision boundary is above the upper bound"
          }
        }
