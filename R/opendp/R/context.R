...
#' to solve for the missing numeric parameter. The parameter is chosen so that
#' the resolved measurement satisfies the query's `d_in` and `d_out`.
#'
#' A query chain may contain at most one unresolved `auto()`. Calling `param()`
#' or `resolve()` performs the same search as `release()`, but does not execute
#' the query or consume privacy budget.
#'
#' @concept context
...
toString.opendp_query <- function(x, ...) {
  chain <- if (is_space(x$chain)) {
    paste0("(", toString(x$chain[[1]]), ", ", toString(x$chain[[2]]), ")")
  } else {
    toString(x$chain)
  }
...
normalize_measure_distance <- function(dist) {
  if (is.list(dist) && all(vapply(dist, is_atomic_distance, logical(1)))) {
    dist <- unlist(dist, use.names = FALSE)
  }
  if (length(dist) == 1) {
    dist[[1]]
  } else {
...
#'
#' Use `Context$compositor(...)` to bind private data to an interactive
#' accountant. Queries built with `query()` are executed with `release()`, and
#' the context tracks the resulting privacy expenditure.
#'
#' @section Accounting modes:
#' Supplying `split_evenly_over` or `split_by_weights` creates a static sequence
#' of per-query allowances. Without a split, a finite `privacy_loss` creates a
#' privacy filter and each query may declare its own loss. An infinite loss
#' creates an odometer that tracks accumulated loss without a finite remaining
#' budget.
#'
#' @section Query lifecycle:
#' Pipe a query through the existing `then_*` constructors. Query builders are
#' immutable: each constructor returns a new query. Building or resolving a
#' query does not consume privacy budget; only a successful `release()` does.
#'
#' @concept context
#' @export
...
#'
#' Static-split contexts assign the next allowance automatically and reject
#' privacy arguments in `...`. Filter and odometer contexts accept arguments
#' understood by `loss_of()`, such as `epsilon`, `delta`, or `rho`. Omitting
#' them leaves `d_out` unspecified, which is useful when the completed
#' measurement has a concrete privacy map.
#'
#' @concept context
#' @param x A context.
#' @param ... Optional privacy loss parameters passed to `loss_of()`.
#' @return A new `opendp_query`.
#' @export
query <- function(x, ...) {
  UseMethod("query")
}
...
#' Release a query.
#'
#' Resolves any pending `auto()` parameter, executes the resulting measurement,
#' and records its privacy loss in the parent context. A failed release does not
#' append to the R-side accounting history.
#'
#' @concept context
#' @param x A query.
...
#' @param bounds Optional search bounds for `auto()`.
#' @param .T Parameter type, either `"float"` or `"int"`.
#' @return The differentially private release, or a child context for a
#'   compositor query.
#' @export
...
#'
#' Transformations already present in the query define the child context's
#' input space and map the parent's `d_in` into the child's metric. When an
#' unbounded child compositor is nested in a bounded parent query, the child is
#' backed by an odometer while the parent reserves the query's finite allowance.
#'
#' @concept context
#' @param x A query.
...
#'
#' Static-split contexts return the allowances consumed so far. Filter and
#' odometer contexts return the accountant's current loss. Tuple-valued losses,
#' such as approximate DP, are returned as numeric vectors.
#'
#' @concept context
#' @param x A context.
#' @return The privacy loss consumed so far.
#' @export
...
current_privacy_loss.opendp_context <- function(x) {
  if (inherits(x$queryable, "odometer_queryable")) {
    return(normalize_measure_distance(x$queryable(d_in = x$d_in)))
  }
  x$d_mids_consumed
}
...
#'
#' Static-split contexts return the unconsumed allowances. Finite privacy
#' filters return the total loss minus the current loss. Unbounded odometers
#' raise an error because there is no finite remaining budget.
#'
#' @concept context
#' @param x A context.
#' @return The remaining privacy loss.
#' @export
...
remaining_privacy_loss.opendp_context <- function(x) {
  if (inherits(x$queryable, "odometer_queryable")) {
    ...
    return(
      normalize_measure_distance(x$d_out) -
        normalize_measure_distance(x$queryable(d_in = x$d_in))
    )
  }
  x$d_mids
}
