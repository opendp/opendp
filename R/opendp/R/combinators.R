...
make_chain_ot <- function(
  ...
  # No type arguments to standardize.
  log <- new_constructor_log("make_chain_ot", "combinators", new_hashtab(
    list("odometer1", "transformation0"),
    list(odometer1, transformation0)
  ))

  # Call wrapper function.
  output <- .Call(
    "combinators__make_chain_ot",
    odometer1, transformation0,
    log, PACKAGE = "opendp")
  output
}

#' partial chain ot constructor
#'
#' See documentation for [make_chain_ot()] for details.
#'
#' @concept combinators
#' @param lhs The prior transformation or metric space.
#' @param odometer1 outer odometer
...
#' @return Odometer
#' @export
then_chain_ot <- function(
  lhs,
  odometer1,
  transformation0
) {

  log <- new_constructor_log("then_chain_ot", "combinators", new_hashtab(
    list("odometer1", "transformation0"),
    list(odometer1, transformation0)
  ))
...
#' @param odometer0 undocumented
#' @return Odometer
#' @export
make_chain_po <- function(
  function1,
  odometer0
) {
  assert_features("contrib")
  ...
  # No type arguments to standardize.
  log <- new_constructor_log("make_chain_po", "combinators", new_hashtab(
    list("function1", "odometer0"),
    list(function1, odometer0)
  ))

  # Call wrapper function.
  output <- .Call(
    "combinators__make_chain_po",
    function1, odometer0,
    log, PACKAGE = "opendp")
  output
}

#' partial chain po constructor
#'
#' See documentation for [make_chain_po()] for details.
#'
#' @concept combinators
#' @param lhs The prior transformation or metric space.
#' @param function1 outer function
...
#' @return Odometer
#' @export
then_chain_po <- function(
  lhs,
  function1,
  odometer0
) {

  log <- new_constructor_log("then_chain_po", "combinators", new_hashtab(
    list("function1", "odometer0"),
    list(function1, odometer0)
  ))
...
#' sequential odometer constructor
#'
...
#' @return Odometer
#' @export
make_sequential_odometer <- function(
  input_domain,
  input_metric,
  output_measure,
  .Q = NULL
  ...
  # Standardize type arguments.
  .Q <- parse_or_infer(type_name = .Q, public_example = get_atom(get_type(output_measure)))

  log <- new_constructor_log("make_sequential_odometer", "combinators", new_hashtab(
    list("input_domain", "input_metric", "output_measure", "Q"),
    list(input_domain, input_metric, output_measure, .Q)
  ))
  ...
  # Call wrapper function.
  output <- .Call(
    "combinators__make_sequential_odometer",
    input_domain, input_metric, output_measure, .Q,
    log, PACKAGE = "opendp")
  output
}

#' partial sequential odometer constructor
#'
#' See documentation for [make_sequential_odometer()] for details.
#'
#' @concept combinators
#' @param lhs The prior transformation or metric space.
#' @param input_domain indicates the space of valid input datasets
...
#' @return Odometer
#' @export
then_sequential_odometer <- function(
  lhs,
  input_domain,
  input_metric,
  output_measure,
  ...
  log <- new_constructor_log("then_sequential_odometer", "combinators", new_hashtab(
    list("input_domain", "input_metric", "output_measure", "Q"),
    list(input_domain, input_metric, output_measure, .Q)
  ))
