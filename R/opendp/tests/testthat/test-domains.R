...
test_that("vector_domain", {
  ...
  expect_true(domain(member = c(1L, 2L)))
  ...
  expect_true(domain(member = rep(1, 12L)))
  expect_false(domain(member = rep(1, 11L)))
  expect_false(domain(member = rep(3, 12L)))
})

test_that("map_domain", {
  domain <- map_domain(atom_domain(.T = "String"), atom_domain(.T = "f64"))
  expect_true(domain(member = new_hashtab(c("A", "B"), c(3, 4))))
