...
test_that("test_sequential_odometer", {
  ...
  o_sc <- space |> then_fully_adaptive_composition(max_divergence(), max_influence)
  ...
  expect_gt(oqbl_sc("privacy_loss"), m_sum(d_in = max_influence) + 0.2 + 0.09)
  ...
  # it's slightly larger, checking greater than will do
  expect_gt(oqbl_sc("privacy_loss"), m_sum(d_in = max_influence) * 2 + 0.2 + 0.09)
})
