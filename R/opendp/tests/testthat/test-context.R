...
test_that("param resolves without consuming privacy budget", {
  context <- Context$compositor(
    data = c(1L, 2L, 3L),
    privacy_unit = unit_of(contributions = 1L),
    privacy_loss = loss_of(epsilon = 1.0)
  )
  q <- query(context, epsilon = 0.5) |>
    then_count() |>
    then_laplace(auto())

  expect_equal(param(q, bounds = c(0.1, 10), .T = "float"), 2)
  expect_equal(param(q, bounds = c(0.1, 10), .T = "float"), 2)
  expect_equal(current_privacy_loss(context), 0)
  expect_equal(remaining_privacy_loss(context), 1)

  expect_type(release(q, bounds = c(0.1, 10), .T = "float"), "integer")
  expect_equal(current_privacy_loss(context), 0.5)
})
...
test_that("fully adaptive approximate DP filter normalizes tuple distances", {
  context <- Context$compositor(
    data = c(1L, 2L, 3L),
    privacy_unit = unit_of(contributions = 1L),
  ...
  expect_equal(current_privacy_loss(context), c(0, 0))
  expect_equal(remaining_privacy_loss(context), c(1, 1e-8))
  expect_error(
    query(context, epsilon = 0.5),
  ...
  expect_type(release(q), "integer")
  expect_equal(current_privacy_loss(context), c(0.5, 0))
  expect_equal(remaining_privacy_loss(context), c(0.5, 1e-8))
})

test_that("unbounded odometer accounts for explicit and inferred query losses", {
  expect_warning(
    loss_of(epsilon = Inf),
    "epsilon should be less than or equal to 5"
  )
  unbounded_loss <- suppressWarnings(loss_of(epsilon = Inf))
  context <- Context$compositor(
    data = c(1L, 2L, 3L),
    privacy_unit = unit_of(contributions = 1L),
    privacy_loss = unbounded_loss
  )

  expect_s3_class(context$accountant, "odometer")
  expect_equal(current_privacy_loss(context), 0)
  expect_error(remaining_privacy_loss(context), "privacy loss is unbounded")

  explicit <- query(context, epsilon = 0.25) |>
    then_count() |>
    then_laplace(auto())
  expect_equal(param(explicit, .T = "float"), 4)
  expect_type(release(explicit), "integer")
  expect_equal(current_privacy_loss(context), 0.25)

  inferred <- query(context) |>
    then_count() |>
    then_laplace(4)
  expect_type(release(inferred), "integer")
  expect_equal(current_privacy_loss(context), 0.5)
  expect_error(remaining_privacy_loss(context), "privacy loss is unbounded")
})
...
test_that("cast_measure covers supported output measure conversions", {
  ...
  pure_dp <- space |> then_laplace(2)
  zcdp <- space |> then_gaussian(2)
  approx_zcdp <- make_approximate(space |> then_gaussian(2.0))

  cases <- list(
    list(
      chain = pure_dp,
      measure = approximate(max_divergence()),
      distance = c(1, 0),
      expected = "Approximate(MaxDivergence)"
    ),
    list(
      chain = zcdp,
      measure = approximate(zero_concentrated_divergence()),
      distance = c(1, 0),
      expected = "Approximate(ZeroConcentratedDivergence)"
    ),
    list(
      chain = pure_dp,
      measure = zero_concentrated_divergence(),
      distance = 1,
      expected = "ZeroConcentratedDivergence"
    ),
    list(
      chain = zcdp,
      measure = approximate(max_divergence()),
  ...
  for (case in cases) {
    cast <- cast_measure_(case$chain, case$measure, case$distance)
    expect_equal(toString(cast("output_measure")), case$expected)
  }
})

test_that("compositor validates incompatible and missing budget arguments", {
  expect_error(
    Context$compositor(
      data = c(1L, 2L, 3L),
      privacy_unit = unit_of(contributions = 1L),
  ...
  expect_error(compositor(q), "d_out is unknown")
})

test_that("nested compositor maps d_in through its transformation chain", {
  context <- Context$compositor(
    data = c(1L, 2L, 3L),
    privacy_unit = unit_of(contributions = 2L),
    privacy_loss = loss_of(epsilon = 1),
    split_evenly_over = 1L
  )
  ...
  expect_equal(subcontext$d_in, 20L)
  sum_query <- query(subcontext) |> then_laplace(auto())
  expect_equal(param(sum_query, .T = "float"), 20)
})
