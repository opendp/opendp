...
test_that("test_gaussian_curve", {
  ...
  meas <- make_zCDP_to_multiDP(input_space |> then_gaussian(4.))
  curve <- meas(d_in = 1.)
  ...
  curve <- make_zCDP_to_multiDP(input_space |> then_gaussian(4.))(d_in = 0.0)
  expect_equal(curve(delta = 0.0), 0.0)

  curve <- make_zCDP_to_multiDP(input_space |> then_gaussian(0.))(d_in = 1.0)
  ...
  curve <- make_zCDP_to_multiDP(input_space |> then_gaussian(0.))(d_in = 0.0)
  expect_equal(curve(delta = 0.0), 0.0)
...
test_that("test_gaussian_search", {
  ...
  make_approx_gauss <- function(scale, delta) {
    make_fix_delta(make_zCDP_to_multiDP(input_space |> then_gaussian(scale)), delta)
  }
...
test_that("test_gaussian_profile_dp", {
  input_space <- c(atom_domain(.T = f64, nan = FALSE), absolute_distance(.T = f64))
  meas <- make_zCDP_to_multiDP(input_space |> then_gaussian(scale = 10.5))
  meas(arg = 100.)
