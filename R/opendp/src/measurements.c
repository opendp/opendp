...
SEXP measurements__make_private_quantile(
    SEXP input_domain, SEXP input_metric, SEXP candidates, SEXP alpha, SEXP scale, SEXP T, SEXP T_candidates, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(input_domain);
    PROTECT(input_metric);
    ...
    // Call library function.
    FfiResult_____AnyMeasurement _result = opendp_measurements__make_private_quantile(c_input_domain, c_input_metric, c_candidates, c_alpha, c_scale);
