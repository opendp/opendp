...
SEXP measurements__make_base_discrete_exponential(
    SEXP input_domain, SEXP input_metric, SEXP temperature, SEXP optimize, SEXP QO, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(input_domain);
    PROTECT(input_metric);
    ...
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
    AnyObject * c_temperature = sexp_to_anyobjectptr(temperature, QO);
    char * c_optimize = (char *)CHAR(STRING_ELT(optimize, 0));
    char * c_QO = rt_to_string(QO);

    // Call library function.
    FfiResult_____AnyMeasurement _result = opendp_measurements__make_base_discrete_exponential(c_input_domain, c_input_metric, c_temperature, c_optimize, c_QO);
