...
SEXP measurements__make_alp_queryable(
    ...
    char * c_CI = rt_to_string(CI);
    char * c_T_value_limit = rt_to_string(T_value_limit);
    char * c_T_size_factor = rt_to_string(T_size_factor);
    char * c_T_alpha = rt_to_string(T_alpha);
...
SEXP measurements__make_geometric(
    ...
    char * c_OptionT = rt_to_string(OptionT);
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
    double c_scale = Rf_asReal(scale);
...
SEXP measurements__make_user_measurement(
    ...
    // Call library function.
    FfiResult_____AnyMeasurement _result = opendp_measurements__make_user_measurement(c_input_domain, c_input_metric, c_output_measure, &c_function, &c_privacy_map, c_TO);
