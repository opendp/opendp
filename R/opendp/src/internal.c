...
SEXP internal___binary_search(
    SEXP predicate, SEXP lower, SEXP upper, SEXP return_sign, SEXP T, SEXP T_predicate, SEXP T_lower, SEXP T_upper, SEXP log
) {
    ...
    char * c_T_lower = rt_to_string(T_lower);
    char * c_T_upper = rt_to_string(T_upper);
    CallbackFn c_predicate = sexp_to_callbackfn(predicate, c_T_predicate);
    void * c_lower = sexp_to_voidptr(lower, T_lower);
    void * c_upper = sexp_to_voidptr(upper, T_upper);
    bool c_return_sign = asLogical(return_sign);

    // Call library function.
    FfiResult_____AnyObject _result = opendp_internal___binary_search(&c_predicate, c_lower, c_upper, c_return_sign, c_T);
