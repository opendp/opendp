...
SEXP transformations__make_count_by(
    SEXP input_domain, SEXP input_metric, SEXP TV, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(input_domain);
    PROTECT(input_metric);
    ...
    // Call library function.
    FfiResult_____AnyTransformation _result = opendp_transformations__make_count_by(c_input_domain, c_input_metric, c_TV);
...
SEXP transformations__make_count_by_categories(
    ...
    char * c_TOA = rt_to_string(TOA);
    char * c_TIA = rt_to_string(TIA);
    char * c_T_categories = rt_to_string(T_categories);
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
...
SEXP transformations__make_create_dataframe(
    ...
    char * c_T_col_names = rt_to_string(T_col_names);
...
SEXP transformations__make_find_bin(
    ...
    char * c_TIA = rt_to_string(TIA);
    char * c_T_edges = rt_to_string(T_edges);
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
...
SEXP transformations__make_impute_constant(
    ...
    char * c_T_constant = rt_to_string(T_constant);
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
...
SEXP transformations__make_quantile_score_candidates(
    ...
    char * c_TIA = rt_to_string(TIA);
    char * c_T_candidates = rt_to_string(T_candidates);
    AnyDomain * c_input_domain = sexp_to_anydomainptr(input_domain);
    AnyMetric * c_input_metric = sexp_to_anymetricptr(input_metric);
...
SEXP transformations__make_quantiles_from_counts(
    ...
    char * c_T_bin_edges = rt_to_string(T_bin_edges);
    char * c_T_alphas = rt_to_string(T_alphas);
...
SEXP transformations__make_split_dataframe(
    ...
    char * c_K = rt_to_string(K);
    char * c_T_col_names = rt_to_string(T_col_names);
    char * c_separator = Rf_isNull(separator) ? NULL : (char *)CHAR(STRING_ELT(separator, 0));
    AnyObject * c_col_names = sexp_to_anyobjectptr(col_names, T_col_names);
...
SEXP transformations__make_subset_by(
    ...
    char * c_TK = rt_to_string(TK);
    char * c_T_keep_columns = rt_to_string(T_keep_columns);
...
SEXP transformations__make_user_transformation(
    ...
    // Call library function.
    FfiResult_____AnyTransformation _result = opendp_transformations__make_user_transformation(c_input_domain, c_input_metric, c_output_domain, c_output_metric, &c_function, &c_stability_map);
