...
SEXP domains___extrinsic_domain_descriptor(
    SEXP domain, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(domain);
    ...
    // Call library function.
    FfiResult_____ExtrinsicObject _result = opendp_domains___extrinsic_domain_descriptor(c_domain);
...
SEXP domains___member(
    ...
    char * c_T_val = rt_to_string(T_val);
    AnyDomain * c_this = sexp_to_anydomainptr(this);
    AnyObject * c_val = sexp_to_anyobjectptr(val, T_val);

    // Call library function.
    FfiResult_____c_bool _result = opendp_domains___member(c_this, c_val);
...
SEXP domains__atom_domain(
    ...
    char * c_T_nan = rt_to_string(T_nan);
...
SEXP domains__bitvector_domain(
    ...
    char * c_T_max_weight = rt_to_string(T_max_weight);
...
SEXP domains__vector_domain(
    ...
    char * c_T_size = rt_to_string(T_size);
