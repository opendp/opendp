...
static const char *get_last_error_message(void)
{
    ...
    const char *result = sexp_to_charptr(message);
    UNPROTECT(2);
    return result;
}
...
static FfiResult_____AnyObject *wrap_failure(const char *backtrace)
{
    FfiResult_____AnyObject *result = (FfiResult_____AnyObject *)malloc(sizeof(FfiResult_____AnyObject));
    ...
    err->message = heap_strdup("Continued stack trace from Exception in user-defined function");
...
static FfiResult_____AnyObject *total_cmp_callback(const void *left_ptr, const void *right_ptr)
{
    ...
    if (!R_ToplevelExec(compare_exec, &ctx))
        return wrap_failure(get_last_error_message());
