...
static char *error_variant_from_message(const char *message)
{
    ...
    // Callback errors use the explicit `[ErrorVariant] : message` encoding.
    // Do not infer a variant from arbitrary callback text such as
    // `NumericRangeAbove happened`.
    if (*candidate != '[')
        return heap_strdup("FFI");
    candidate++;

    const char *end = strchr(candidate, ']');
    if (!end || end == candidate)
        return heap_strdup("FFI");

    const char *separator = end + 1;
