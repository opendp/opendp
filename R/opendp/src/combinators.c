...
SEXP combinators__make_adaptive_composition(
    ...
    char * c_T_d_in = rt_to_string(T_d_in);
    char * c_T_d_mids = rt_to_string(T_d_mids);
    ...
    FfiResult_____AnyMeasurement _result = opendp_combinators__make_adaptive_composition(c_input_domain, c_input_metric, c_output_measure, c_d_in, c_d_mids);
...
SEXP combinators__make_basic_composition(
    ...
    char * c_T_measurements = rt_to_string(T_measurements);
...
SEXP combinators__make_composition(
    SEXP measurements, SEXP T_measurements, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(measurements);
    ...
    // Call library function.
    FfiResult_____AnyMeasurement _result = opendp_combinators__make_composition(c_measurements);
...
SEXP combinators__make_fully_adaptive_composition(
    SEXP input_domain, SEXP input_metric, SEXP output_measure, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(input_domain);
    PROTECT(input_metric);
    ...
    // Call library function.
    FfiResult_____AnyOdometer _result = opendp_combinators__make_fully_adaptive_composition(c_input_domain, c_input_metric, c_output_measure);

    UNPROTECT(4);
    if(_result.tag == Err_____AnyOdometer)
        return(extract_error(_result.err));
    AnyOdometer* _return_value = _result.ok;
    return(anyodometerptr_to_sexp(_return_value, log));
}
...
SEXP combinators__make_privacy_filter(
    SEXP odometer, SEXP d_in, SEXP d_out, SEXP T_d_in, SEXP T_d_out, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(odometer);
    PROTECT(d_in);
    ...
    char * c_T_d_in = rt_to_string(T_d_in);
    char * c_T_d_out = rt_to_string(T_d_out);
    AnyOdometer * c_odometer = sexp_to_anyodometerptr(odometer);
    AnyObject * c_d_in = sexp_to_anyobjectptr(d_in, T_d_in);
    AnyObject * c_d_out = sexp_to_anyobjectptr(d_out, T_d_out);

    // Call library function.
    FfiResult_____AnyMeasurement _result = opendp_combinators__make_privacy_filter(c_odometer, c_d_in, c_d_out);
