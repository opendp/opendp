...
SEXP core__measurement_check(
    ...
    char * c_T_distance_in = rt_to_string(T_distance_in);
    char * c_T_distance_out = rt_to_string(T_distance_out);
...
SEXP core__new_function(
    ...
    // Call library function.
    FfiResult_____AnyFunction _result = opendp_core__new_function(&c_function, c_TO);
...
SEXP core__new_queryable(
    ...
    char * c_T_transition = rt_to_string(T_transition);
    TransitionFn c_transition = sexp_to_transitionfn(transition, c_T_transition);

    // Call library function.
    FfiResult_____AnyObject _result = opendp_core__new_queryable(&c_transition, c_Q, c_A);

    transitionfn_release(&c_transition);
...
SEXP core__odometer_queryable_invoke(
    ...
    char * c_T_query = rt_to_string(T_query);
