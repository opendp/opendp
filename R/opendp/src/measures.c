...
SEXP measures___extrinsic_measure_descriptor(
    ...
    // Call library function.
    FfiResult_____ExtrinsicObject _result = opendp_measures___extrinsic_measure_descriptor(c_measure);

    UNPROTECT(2);
    if(_result.tag == Err_____ExtrinsicObject)
        return(extract_error(_result.err));
    ExtrinsicObject* _return_value = _result.ok;
    return(extrinsic_object_to_sexp(_return_value));
}
...
SEXP measures__new_privacy_profile(
    SEXP curve, SEXP T_curve, SEXP log
) {
    // Convert arguments to c types.
    PROTECT(curve);
    PROTECT(T_curve);
    ...
    char * c_T_curve = rt_to_string(T_curve);
    CallbackFn c_curve = sexp_to_callbackfn(curve, c_T_curve);

    // Call library function.
    FfiResult_____AnyObject _result = opendp_measures__new_privacy_profile(&c_curve);

    callbackfn_release(&c_curve);
...
SEXP measures__user_divergence(
    ...
    // Call library function.
    FfiResult_____AnyMeasure _result = opendp_measures__user_divergence(c_identifier, c_descriptor);
