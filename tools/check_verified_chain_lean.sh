...
crate_root="$git_root/rust/opendp_verified"   # lake package root == the verified crate
proj_dir="$crate_root"

# 1. No `sorry` tokens in handwritten sources. Backtick-quoted doc mentions
#    (`sorry`) are excluded by the character classes.
if grep -rn --include='*.lean' -E '(^|[^`A-Za-z_])sorry([^`A-Za-z_]|$)' "$crate_root/src"; then
...
if grep -rn --include='*.lean' -E '^[[:space:]]*axiom[[:space:]]' "$crate_root/src"     | grep -vE "src/($allowed_axiom_files):"; then
