...
def initialize(args):
    ...
    if args.sync:
        ...
        upstream = (
            channel_to_upstream[args.channel]
        ...
        if args.preserve:
            ...
            run_command(
                "Creating temporary branch based on upstream",
                f"git switch -c tmp {upstream}",
            )
            run_command(
                "Merging channel (keeping all upstream)",
                f"git merge -s ours {args.channel}",
            )
            ...
            run_command(
                "Resetting channel to upstream",
                f"git switch -C {args.channel} {upstream}",
            )
    else:
...
def infer_counter(version, date, args):
    ...
    if version.prerelease is None:
        return 1
    match = re.match(rf"^{args.channel}\.(\d+)\.(\d+)", version.prerelease)
    if match is None:
        return 1
...
def update_version(version):
    ...
    update_file(
        "python/setup.cfg", load_python_config, munge_python_config, dump_python_config
    )
...
def match_first_changelog_header(lines):
    pattern = rf"^## \[(\d+\.\d+\.\d+(?:-\S+)?)\]\({re.escape(URL_BASE)}(\S+)\.\.\.\S+\) - \S+$"
...
def changelog(args):
    ...
    if args.prepend:
        ...
        if version.finalize_version() <= heading_version.finalize_version():
            raise Exception(
                f"Prepending new heading, but VERSION {version} hasn't been bumped above heading version {heading_version}"
            )
        ...
        if version.finalize_version() != heading_version.finalize_version():
            raise Exception(
                f"VERSION {version} isn't compatible with heading version {heading_version}"
            )
        new_heading_version = version
        diff_target = (
            f"v{version}"
            if channel == "stable"
            else (channel if channel != "dev" else "HEAD")
        )
        if channel == "dev":
            date = "TBD"

    log(
        f"Updating heading to {new_heading_version}, {diff_source}...{diff_target}, {date}"
    )
    changelog_lines[i] = (
        f"## [{new_heading_version}]({URL_BASE}{diff_source}...{diff_target}) - {date}\n"
    )
    if args.prepend:
        ...
        changelog_lines[i:i] = [
            f"## [{version}]({URL_BASE}{diff_source}...HEAD) - TBD\n",
            "\n",
            "\n",
        ]
...
def _main(argv):
    ...
    subparser.add_argument(
        "-c",
        "--channel",
        choices=["dev", "nightly", "beta", "stable"],
        default="nightly",
        help="Which channel to target",
    )
    subparser.add_argument(
        "--sync", action="store_true", help="Sync the channel from upstream"
    )
    ...
    subparser.add_argument(
        "-i", "--counter", type=int, default=0, help="Intra-date version counter"
    )
    ...
    subparser.add_argument(
        "--prepend",
        action="store_true",
        help="Prepend new empty heading (for dev only)",
    )

    subparser = subparsers.add_parser(
        "bump_version", help="Bump the version number (assumes dev channel)"
    )
    subparser.set_defaults(func=bump_version)
    subparser.add_argument(
        "-p", "--position", choices=["major", "minor", "patch"], default="minor"
    )
