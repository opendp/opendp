...
def rust(args):
    ...
    run_command(
        "Publishing opendp_tooling crate",
        f"cargo publish{dry_run_arg} --verbose --manifest-path=rust/opendp_tooling/Cargo.toml",
    )
    if not args.dry_run:
        ...
        run_command(
            "Publishing opendp_derive crate",
            "cargo publish --verbose --manifest-path=rust/opendp_derive/Cargo.toml",
        )
        # We've expanded default-members so "cargo test" tests more packages,
        # but that means we need to be explicit with "cargo publish".
        run_command(
            "Publishing opendp crate",
            "cargo publish --verbose --manifest-path=rust/Cargo.toml --package opendp",
        )
...
def python(args):
    ...
    run_command(
        "Publishing opendp package",
        f"python -m twine upload -r {args.repository} --verbose python/dist/*",
    )
...
def sanity(args):
    ...
    if args.python_repository == "local":
        package_name = f"opendp-{version}-cp310-abi3-manylinux_2_28_x86_64.whl"
        package = f"python/dist/{package_name}"
        run_command(
            f"Installing opendp {version}",
            f". {args.venv}/bin/activate && pip install {package}",
        )
    else:
        index_url = (
            "https://test.pypi.org/simple"
            if args.python_repository == "testpypi"
            else "https://pypi.org/simple"
        )
        package = f"opendp=={version}"
        run_command_with_retries(
            f"Installing opendp {version}",
            f". {args.venv}/bin/activate && pip install -i {index_url} {package}",
            args.package_timeout,
            args.package_backoff,
        )
    if args.fake:
        run_command(
            "Running test script", f". {args.venv}/bin/activate && echo FAKE TEST!!!"
        )
    else:
        run_command(
            "Running test script", f". {args.venv}/bin/activate && python tools/test.py"
        )
...
def github(args):
    ...
    if branch != channel:
        raise Exception(
            f"Version {version} implies channel {channel}, but current branch is {branch}"
        )
    tag = f"v{version}"
...
def _main(argv):
    ...
    subparser.add_argument(
        ...
        type=int,
        default=300,
        help="How long to keep checking for index update (0 = don't check)",
    )
    subparser.add_argument(
        "-b",
        "--index-check-backoff",
        type=float,
        default=2.0,
        help="How much to back off between checks for index update",
    )
    ...
    subparser.add_argument(
        "-e", "--venv", default="/tmp/sanity-venv", help="Virtual environment directory"
    )
    subparser.add_argument(
        "-r",
        "--python-repository",
        choices=["pypi", "testpypi", "local"],
        default="pypi",
        help="Python package repository",
    )
    subparser.add_argument(
        "-t",
        "--package-timeout",
        type=int,
        default=0,
        help="How long to retry package installation attempts (0 = no retries)",
    )
    subparser.add_argument(
        "-b",
        "--package-backoff",
        type=float,
        default=2.0,
        help="How much to back off between package installation attempts",
    )
    ...
    subparser.add_argument(
        "-d", "--date", type=datetime.date.fromisoformat, help="Release date"
    )
